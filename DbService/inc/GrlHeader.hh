...
class GrlHeader {
 public:
  ...
  GrlHeader(const std::string& name) :
      _name(name), _locked(0), _lid(-1), _create_time(""), _create_user("") {}
  GrlHeader(const std::string& name, int locked, int lid,
            std::string const& create_time, std::string const& create_user) :
      _name(name),
