...
mu2e::DbIdList::DbIdList() {
  ...
  if (!in) {
    throw cet::exception("DBID_NO_CONNECTION")
        << "DbIdList::DbIdList could not open connections file " << fn
        << std::endl;
  }
  ...
  while (in) {
    ...
    // if eof, or starting a new database stanza with a previous database name
    // filled
    if (in.eof() ||
        (words.size() == 2 && words[0] == "database" && !name.empty())) {
      // have read a stanza, check and store it
      bool fail =
          host.empty() || port.empty() || url.empty() || urlNoCache.empty();
      if (fail) {
