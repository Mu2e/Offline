...
mu2e::DbReader::DbReader() :
    _curl_handle(nullptr), _timeout(3600), _totalTime(0), _removeHeader(true),
    _abortOnFail(true), _useCache(true), _cacheLifetime(0), _verbose(0),
    _timeVerbose(0), _saveCsv(true) {
...
int mu2e::DbReader::query(std::string& csv, const std::string& select,
                          const std::string& table, const StringVec& where,
...
int mu2e::DbReader::multiQuery(std::vector<QueryForm>& qfv) {
  ...
  while (rc == 0 && iter != qfv.end()) {
    rc = queryCore(iter->csv, iter->select, iter->table, iter->where,
                   iter->order);
    iter++;
  }
...
int mu2e::DbReader::fillValTables(DbValCache& vcache) {
  ...
  _lastTime = std::chrono::duration_cast<std::chrono::microseconds>(end_time -
                                                                    start_time);
  _totalTime += _lastTime;

  if (_timeVerbose > 1) {
    std::cout << "DbReader::fillValCache took " << std::setprecision(6)
              << _lastTime.count() * 1.0e-6 << " s" << std::endl;
  }
  if (_verbose > 2) {
...
int mu2e::DbReader::queryCore(std::string& csv, const std::string& select,
                              const std::string& table, const StringVec& where,
                              const std::string& order) {
  std::string url;
  bool qcache = true;
  ...
  if (_verbose > 5) {
    ...
    std::cout << "DbReader " << time << "  url=" << url << std::endl;
  }
  ...
  while (_curl_ret != CURLE_OK && nsec < _timeout) {
    ...
    _result.reply.clear();
    if (_verbose > 5)
      std::cout << "DbReader start perform with itry=" << itry << std::endl;
    ...
    if (_curl_ret == CURLE_OK && http_code == 200) {
      ...
      if (_verbose > 0)
        std::cout << "DbReader try: " << itry << "   lastError: " << _lastError
                  << "   http code: " << http_code << std::endl;
    }

    auto try_time = std::chrono::high_resolution_clock::now();
    nsec = (std::chrono::duration_cast<std::chrono::seconds>(try_time -
                                                             start_time))
               .count();
  ...
  if (_timeVerbose > 5 || _verbose > 5) {
    ...
    std::cout << "DbReader " << time << " " << std::setprecision(6)
              << _lastTime.count() * 1.0e-6 << " s "
              << "to read " << table << " on try " << itry << " with "
