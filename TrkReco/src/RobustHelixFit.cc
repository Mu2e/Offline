...
namespace mu2e
{
  ...
  void RobustHelixFit::fitCircleMedian(RobustHelixFinderData& helixData) 
  {
    ...
    for (const auto& hhit : hhits) {
      if (use(hhit) ){
	wpos.push_back(XYWVec(hhit.pos(),hhit.sid().uniqueFace(),hhit.nStrawHits()));
      }
    }
    XYZVec cog;
	...
	cog = Geom::toXYZVec(calo->geomUtil().mu2eToTracker(calo->geomUtil().diskFFToMu2e(helixData._hseed->caloCluster()->diskId(),helixData._hseed->caloCluster()->cog3Vector())));
	wpos.push_back(XYWVec(cog,-1,_ccwt));//faceId=-1 for the calorimeter
      }
    ...
    unsigned ntriple(0);
    size_t np = wpos.size();    
    int    faceP1(-1), faceP2(-1), faceP3(-1);
    for(size_t ip=0; ip<np-2; ++ip)
      { 
	faceP1    = wpos[ip].faceId();
	float ri2 = wpos[ip].Mag2();
	for(size_t jp=ip+1; jp<np-1; ++jp)
	  {                
	    faceP2        = wpos[jp].faceId();
	    if (faceP2 == faceP1)                   continue;
	    ...
	    for(size_t kp=jp+1; kp<np; ++kp)
	      {
		faceP3        = wpos[kp].faceId();
		if((faceP3 == faceP2) || (faceP3 == faceP1))                                   continue;
