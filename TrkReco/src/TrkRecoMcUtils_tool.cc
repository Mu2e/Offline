...
public:
  ...
  const XYZVectorF getMomentum(const art::Event* Event, int Index);
  const CLHEP::Hep3Vector getPosition(const art::Event* Event, int Index);
  const CLHEP::Hep3Vector getStartPosition(const SimParticle* Sim);
  // int               Index,
...
int TrkRecoMcUtils::initEvent(const art::Event* Event) {
  ...
  if (mcdigiH.isValid())
    _mcdigis = (mu2e::StrawDigiMCCollection*)mcdigiH.product();
  else
    _mcdigis = NULL;
...
const XYZVectorF TrkRecoMcUtils::getMomentum(const art::Event* Event, int Index) {
  if (Event->event() != _lastEvent)
    initEvent(Event);
  return (*_mcdigis)[Index].earlyStrawGasStep()->momentum();
}

const CLHEP::Hep3Vector TrkRecoMcUtils::getPosition(const art::Event* Event, int Index) {
  if (Event->event() != _lastEvent)
    initEvent(Event);
  return (*_mcdigis)[Index].earlyStrawGasStep()->position();
}

const CLHEP::Hep3Vector TrkRecoMcUtils::getStartPosition(const SimParticle* Sim) {
  return Sim->startPosition();
}
...
//-----------------------------------------------------------------------------
// const StrawDigiMCCollection* TrkRecoMcUtils::getListOfMcStrawHits(const art::Event* Event,const
// art::InputTag& Tag) {
