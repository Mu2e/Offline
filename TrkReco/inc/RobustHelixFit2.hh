...
//
#ifndef TrkReco_RobustHelixFit2_HH
#define TrkReco_RobustHelixFit2_HH
...
// (s)ums of (c)osine and (s)in for points on (c)ircumference, (o)utside the median radius, or (i)nside the median radius
  ...
  class RobustHelixFit2
  {
  public:
  // circle fit types
    enum CircleFit {median=0,AGE=1, chisq=2 };
// parameter set should be passed in on construction
    explicit RobustHelixFit2(fhicl::ParameterSet const&);
    virtual ~RobustHelixFit2();
...
// the HelixSeed object is both input and output.  This allows for iterative calling
    void fitHelix(HelixSeed& myfit, const CaloRecoDigiFastCollection& caloDigis);
    // alternatively, user can fit the two views separately to allow hit cleaning between
    void fitCircle(HelixSeed& hseed, const CaloRecoDigiFastCollection& caloDigis);
...
// Helicity of this fit
  ...
  private:
    ...
    void fitCircleMedian(HelixSeed& hseed, const CaloRecoDigiFastCollection& caloDigis);
    void fitCircleAGE(HelixSeed& hseed, const CaloRecoDigiFastCollection& caloDigis);
