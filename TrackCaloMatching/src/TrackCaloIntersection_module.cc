...
namespace mu2e {
  ...
  private:
    ...
    art::ProductToken<KalRepPtrCollection> const _trkterToken;
    bool                          _downstream;
    int                           _diagLevel;
    double                        _pathStep;
    double                        _tolerance;
  ...
  {
    //get tracks
    auto const& trksPtrColl = *evt.getValidHandle(_trkterToken);

    //output of Extrapolated tracks
    auto extrapolatedTracks = std::make_unique<TrkCaloIntersectCollection>();
