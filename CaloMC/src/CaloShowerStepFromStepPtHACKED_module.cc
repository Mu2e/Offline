...
namespace mu2e {
  ...
  class CaloShowerStepFromStepPtHACKED : public art::EDProducer {

       public:

          explicit CaloShowerStepFromStepPtHACKED(fhicl::ParameterSet const& pset) :
             art::EDProducer{pset},
          ...
          virtual      ~CaloShowerStepFromStepPtHACKED() {}
    ...
    //--------------------------------------------------------------------
    void CaloShowerStepFromStepPtHACKED::beginJob()
    {            
    ...
    void CaloShowerStepFromStepPtHACKED::beginSubRun(art::SubRun& sr) 
    {
        mapPhysVol_.clear();
    ...
    //--------------------------------------------------------------------
    void  CaloShowerStepFromStepPtHACKED::produce(art::Event& event)
    {
        
        totalEdep_=0.0;
        totalStep_=0;
        
        if ( diagLevel_ > 0) std::cout << "[CaloShowerStepFromStepPtHACKED::produce] begin" << std::endl;
        ...
        //FIXME: // These selectors will select data products with the given instance name, and ignore all other fields of the product ID.
        //FIXME: art::ProductInstanceNameSelector getCrystalSteps(calorimeterStepPoints_);
        //FIXME: art::ProductInstanceNameSelector getReadoutSteps(calorimeterROStepPoints_);
        //FIXME:
        //FIXME: // Get the StepPointMCs from the event.
        //FIXME: HandleVector crystalStepsHandles, readoutStepsHandles;
        //FIXME: //event.getMany( getCrystalSteps, crystalStepsHandles);
        //FIXME: //event.getMany( getReadoutSteps, readoutStepsHandles);

        HandleVector crystalStepsHandles, readoutStepsHandles;
        {
          art::Handle<StepPointMCCollection> hc;
          event.getByLabel(art::InputTag(calorimeterStepPoints_), hc);
          crystalStepsHandles.push_back(hc);

          art::Handle<StepPointMCCollection> hr;
          event.getByLabel(art::InputTag(calorimeterROStepPoints_), hr);
          readoutStepsHandles.push_back(hr);
        }

        makeCompressedHits(crystalStepsHandles,readoutStepsHandles,*caloShowerStepMCs,*caloROShowerStepMCs,*simsToKeep);
 
        if ( diagLevel_ > 0) std::cout << "[CaloShowerStepFromStepPtHACKED::produce] Total energy deposited / number of stepPointMC: " <<totalEdep_<<" / "<<totalStep_<<std::endl;
        if ( diagLevel_ > 0) std::cout << "[CaloShowerStepFromStepPtHACKED::produce] Total sims init: " <<totalSim_<<std::endl;
        if ( diagLevel_ > 0) std::cout << "[CaloShowerStepFromStepPtHACKED::produce] Total caloShower steps: " <<caloShowerStepMCs->size()<<std::endl;
        if ( diagLevel_ > 0) std::cout << "[CaloShowerStepFromStepPtHACKED::produce] end" << std::endl;
    ...
    void CaloShowerStepFromStepPtHACKED::makeCompressedHits(const HandleVector& crystalStepsHandles, 
                                                      ...
                                                      SimParticlePtrCollection& simsToKeep)
    {
         ...
         if (diagLevel_ > 2)
         { 
            std::cout<<"CaloShowerStepFromStepPtHACKED summary"<<std::endl;
         ...
         //Final statistics
         if (diagLevel_ > 1) std::cout<<"[CaloShowerStepFromStepPtHACKED::makeCompressedHits] compressed "<<nCompress<<" / "<<nCompressAll<<" incoming SimParticles"<<std::endl;
         if (diagLevel_ > 1) std::cout<<"[CaloShowerStepFromStepPtHACKED::makeCompressedHits] keeping "<<simsToKeep.size()<<" CaloShowerSteps"<<std::endl;
    ...
    //------------------------------------------------------------------------------------------------------------------
    void CaloShowerStepFromStepPtHACKED::collectStepBySimAncestor(const Calorimeter& cal, 
    ...
    //-------------------------------------------------------------------------------------------------------------------------
    bool CaloShowerStepFromStepPtHACKED::isInsideCalorimeter(const Calorimeter& cal, const PhysicalVolumeMultiHelper& vi, const art::Ptr<SimParticle>& thisSimPtr)
    {        
        if (usePhysVol_) 
    ...
    //-------------------------------------------------------------------------------------------------------------
    void CaloShowerStepFromStepPtHACKED::collectStepBySim(const HandleVector& stepsHandles, 
    ...
    //-------------------------------------------------------------------------------------------------------------------------------
    void CaloShowerStepFromStepPtHACKED::compressSteps(const Calorimeter& cal, CaloShowerStepCollection &caloShowerStepMCs, 
                                                 bool isCrystal, int volId, const SimPtr& sim, std::vector<const StepPointMC*>& steps)
    {
          ...
          {               
		      ...
		      hZpos2_->Fill(idx,buffer.energyDep(idx));
                      ...
                      if (diagLevel_ > 3) {std::cout<<"[CaloShowerStepFromStepPtHACKED::compressSteps] inserted     "; buffer.printBucket(idx);}                      
                  }    
         ...
         for (int i=0;i<buffer.nBuckets();++i)
         {
              ...
              if (diagLevel_ > 2) 
              {
                  ...
                  if (diagLevel_ > 3) {std::cout<<"[CaloShowerStepFromStepPtHACKED::compressSteps] inserted     ";  buffer.printBucket(i);}
              }    
         }                           
    }
    ...
    //------------------------------------------------------------------------------------------------------------------------------------------
    bool CaloShowerStepFromStepPtHACKED::isCompressible(int simPdgId, const std::unordered_set<int>& processCodes, const SimParticlePtrCollection& sims)
    {      
    ...
    //-------------------------------------------------------------------------------------------------------------
    void CaloShowerStepFromStepPtHACKED::dumpAllInfo(const HandleVector& stepsHandles,const Calorimeter& cal)
    {       
...
using mu2e::CaloShowerStepFromStepPtHACKED;
DEFINE_ART_MODULE(CaloShowerStepFromStepPtHACKED);
