...
//
// $Id: ClosestCaloClusterFinder.cc,v 1.1 2012/02/28 22:24:48 gianipez Exp $
// $Author: gianipez $
...
bool ClosestCaloClusterFinder::find( ClusterData &cluster, MatrixCaloHit &data , unsigned int row, unsigned int colum ){
                        ...
                        for (int icl=-1; icl<2; icl++) {
                                ...
                                if ( (ida2 != ida->second.end()) && !looked[tmp_row][tmp_colum] ) {
                                        ...
                                        for(std::vector<std::pair<CaloCrystalHit *, size_t > >::iterator jHit = ida2->second.begin(); jHit != ida2->second.end(); ++jHit ){
                                                if(fabs(HitTimeMin - jHit->first->time()) <= deltaTime  && jHit->first->energyDep() >= noiseCut ){
