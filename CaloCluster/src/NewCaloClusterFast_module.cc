...
namespace mu2e {
  ...
  struct Finder{
        Finder(CaloCrystalHit const*  crystalSeed_, std::list<const CaloCrystalHit*>  clusterList_, std::queue<int>  crystalToVisit_, std::vector<bool>      isVisited_) :
        CaloCrystalHit const*  crystalSeed_;
        std::list<const CaloCrystalHit*>  clusterList_;
  ...
  public:
    ...
    explicit NewCaloClusterFast(fhicl::ParameterSet const& pset) :
      ...
      minSeedEnergy_( pset.get<double>("minSeedEnergy")),
      minHitEnergy_( pset.get<double>("minHitEnergy")),
  ...
  private:
   ...
   void FillClusters(CaloClusterCollection& recoClusters, 
 const art::Handle<CaloCrystalHitCollection>& CaloCrystalHitsHandle, std::vector<CaloCrystalList>& clusterPtrList,  const Calorimeter* cal);
  };
  ...
  void NewCaloClusterFast::produce(art::Event& event)
  {
    ...
    recoClustersColl->reserve(10); //posibble
  ...
  {
    CaloCrystalList List; //should be pointer!!! 
    Finder *f = new Finder();
    ...
    std::vector<CaloCrystalList>       caloIdHitMap(cal->nCrystal()), clusterList;// possible
    std::list<const CaloCrystalHit*>  seeds_;

    //Loop through collection, fill seeds and map:
    for (const auto& hit : recoCrystalHits)
      {
	if (hit.energyDep() < minHitEnergy_) continue;
    ...
    while( !seeds_.empty() )
      {
	//find seed:
	const CaloCrystalHit* crystalSeed = *seeds_.begin(); //possible
	//check it fulfils criteria on energy:
	if (crystalSeed->energyDep() < minSeedEnergy_) break;

	//make some lists for cluster finding: 
	std::cout<<"Start of Forms "<<std::endl;
	
	std::queue<int> crystalToVisit_; //Possible sopurce
	...
	std::vector<bool>      isVisited_; //possible
	isVisited_.reserve(crystalToVisit_.size()); //possible
	...
	while (!crystalToVisit_.empty())
	  {            
		...
		//iterate:
		auto it=list.begin();
		while(list.size()!=0 and it != list.end())
		  {
		    //get hit:
		    ...
		    if (std::abs(hit->time() - seedTime_) < deltaTime_) 
		      { 
			//check energy:
			if (hit->energyDep() < minHitEnergy_) { crystalToVisit_.push(iId); }
			//add hit:
			List.push_front(hit);
			//remove:
			it = list.erase(it);   
	...
	//add list to list-of-lists for cluster making:
	clusterList.push_back(List);
	//delete(List);
	//remove from seeds:
	for (const auto& hit : List) { 
	  if(!seeds_.empty()) seeds_.remove(hit); 
	} 
	
     }//ends seeds //NEVER REACHES HERE
    std::cout<<"end of makes function "<<std::endl;
    FillClusters(recoClusters, CaloCrystalHitsHandle, clusterList, *cal);
    
}

void NewCaloClusterFast::FillClusters(CaloClusterCollection& recoClusters,
 const art::Handle<CaloCrystalHitCollection>& CaloCrystalHitsHandle, std::vector<CaloCrystalList>& clusterList, const Calorimeter* cal)
  {
      ...
      for (auto clusterPrt : cluster) //Note: a clusterPrt is a crystal hit
	{
      ...
      if (totalEnergy < minEnergy_)  continue;
      ...
      recoClusters.emplace_back(std::move(Endcluster)); //Time ordering?
      delete cal;
    } //ends clusters
  }//end function
}
