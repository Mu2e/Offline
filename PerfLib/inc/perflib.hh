...
struct pme_t {
  ...
  pme_t(unsigned int event_, unsigned int umask_, unsigned int cmask_, unsigned int inv_) : event(event_), umask(umask_), inv(inv_),cmask(cmask_) {}

  operator unsigned int() const{ return _raw;}

  union {
    struct {
      /*This  field  is  used  to  select  the  logic  unit to  detect  the  performance  monitoring  event  to  be
      *monitored. So the values to be filled in this field is determined by the architecture.*/
      unsigned int event : 8;

      /*The  logic  unit  selected  by  the  Event  Select  field might  be  capable  of  monitoring multiple events.
      *So this UMASK field is used to select one of those events which can be monitored
      *by the logic unit. So based on the logic unit selected the UMASK field may have one fixed
      *value or multiple values, which is dependent on the architecture.*/
      unsigned int umask : 8;

      /*This  flag
      *,  if  set,  tells  the  logic  unit  to  monitor  events  which  happen  when  the  processor  is
      ...
      *together to monitor or count all the events.*/
      ...
      /*This  flag  when  set  increments  the  counter  and  toggles  the  PMi  pin  when  the  monitored event occurs.
       *And when not set, it toggles the PMi pin only when the counter overflows.*/
      unsigned int pc : 1;

      /*When  this  flag  is  set  the  processor  raises  an  interrupt  when  the  performance  monitoring counter
       * overflows.*/
      unsigned int intr : 1;
      ...
      /*This  flag  when  set  enables  the  performance  monitoring  counters  for  the  event  and  when clear
       * disables
       * the counters.*/
      unsigned int en : 1;

      /*This  flag  when  set  inverts  the  output  of  CMASK  comparison.  This  enables  the  user  to  set both
       * greater than and less than comparisons between CMASK and the counter value.*/
      unsigned int inv : 1;

      /*If this field has a value more tlshan zero then that value is compared to the number of events generated
       * in  one  clock cycle.  If  the  events generated  is  more than the CMASK  value  then the counter is
...
class PerfStats {
 public:
  PerfStats(std::string const& observation_name) { init(); _observation_name=observation_name;}
 ~PerfStats() { stop_counters(); record_counters(); }
