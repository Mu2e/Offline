...
int mu2e::ValKalSeed::declare(const art::TFileDirectory& tfs) {
  ...
  _hSRadLen = tfs.make<TH1D>("SRadLen", "Fractional Straw Radiation Length",
                             100, 0, 1.0e-3);
  _hSRadLenSum = tfs.make<TH1D>(
      "SRadLenSum", "Sum Fractional Straw Radiation Length", 100, 0, 0.04);
  int ibin = 1;
...
double mu2e::ValKalSeed::mcTrkP(art::Event const& event) {
  ...
  for (auto const& ah : vah) {       // loop over all SimParticle coll
    ...
    for (auto sp : simpcoll) {       // loop over SimParticles
      ...
      if (part.pdgId() == 11 &&
          part.creationCode() == ProcessCode::mu2eCeMinusEndpoint) {
