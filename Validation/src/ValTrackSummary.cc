...
int mu2e::ValTrackSummary::fill(const mu2e::TrackSummaryCollection& coll,
  ...
  for (auto trk : coll) {
    ...
    if (!statev.empty()) {
      ...
      _hpce->Fill(p);
      bool d0cut =
          (d0 < 105 && d0 > -80 && (d0 + 2 / om) > 450 && (d0 + 2 / om) < 680);
...
double mu2e::ValTrackSummary::mcTrkP(art::Event const& event) {
  ...
  /*
        ...
        if(vids.size() == 0 ||  (imcs->trackId() == trkid &&
find(vids.begin(),vids.end(),imcs->volumeId()) != vids.end())){
...
const StepPointMCCollection *_mcsteps, *_mcvdsteps
  ...
  KalDiag::findMCSteps(StepPointMCCollection const* mcsteps, cet::map_vector_key
const& trkid, vector<int> const& vids, vector<MCStepItr>& steps) {
    steps.clear();
    if(mcsteps != 0){
      // Loop over the step points, and find the one corresponding to the given
detector for( MCStepItr imcs =mcsteps->begin();imcs!= mcsteps->end();imcs++){
        if(vids.size() == 0 ||  (imcs->trackId() == trkid &&
find(vids.begin(),vids.end(),imcs->volumeId()) != vids.end())){
          steps.push_back(imcs);
        }
