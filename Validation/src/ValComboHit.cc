...
int mu2e::ValComboHit::declare(const art::TFileDirectory& tfs) {
  ...
  _hWD = tfs.make<TH1D>("WD", "WireDistance;Distance from mid (mm)", 100,
                        -800.0, 800.0);
  _hDE = tfs.make<TH1D>("DE", "TOT Drift Time Estimate;Drift Time (ns)", 100,
                        0.0, 80.0);
  ...
  _hwres =
      tfs.make<TH1D>("wres", "Wire resolution;resolution (mm)", 50, 0.0, 100.0);
  _htres = tfs.make<TH1D>("tres", "Transverse resolution;resolution (mm)", 50,
                          0.0, 10.0);
