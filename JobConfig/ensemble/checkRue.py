...
rup_passed = False
for i in range(3):
  rue = float(open("closedEnsemble_test%d/rue" % i).readline())
  rup = float(open("closedEnsemble_test%d/rup" % i).readline())
...
if not rue_passed:
  ...
  fout = open("closedEnsemble_test%d/rue" % whichensemble,"w")
  fout.write("%e\n" % rue)
  fout.close()
...
if not rup_passed:
  ...
  fout = open("closedEnsemble_test%d/rup" % whichensemble,"w")
  fout.write("%e\n" % rup)
  fout.close()
