...
void cluAnaSignalView(int entry)
{
   ...
   for (int i=0;i<10;++i)  _hview1[i]  = new TH2F(Form("hview1_%i",i),      " ",           200, -700,700,200,-700,700);
   for (int i=0;i<10;++i)  _hview2[i]  = new TH2F(Form("hview2_%i",i),      " ",           200, -700,700, 200, 400,2000);
   for (int i=0;i<10;++i)  _hview3[i]  = new TH2F(Form("hview3_%i",i),      " ",           200, -700,700, 200, 400,2000);

   for (int i=0;i<10;++i){     
     _hview1[i]->GetXaxis()->SetTitle("x");
     _hview1[i]->GetYaxis()->SetTitle("y");
     _hview2[i]->GetXaxis()->SetTitle("x");
     _hview2[i]->GetYaxis()->SetTitle("t");
     _hview3[i]->GetXaxis()->SetTitle("y");
     _hview3[i]->GetYaxis()->SetTitle("t");
   ...
   for (int in=0;in<tree->GetEntriesFast();++in){
      ...
      for (int id=0;id<*nindex;++id)
      {
         ...
         _hview1[ipass]->Fill(hitRad[ih]*cos(hitPhi[ih]),hitRad[ih]*sin(hitPhi[ih]));
         _hview2[ipass]->Fill(hitRad[ih]*cos(hitPhi[ih]),hitTime[ih]);
         _hview3[ipass]->Fill(hitRad[ih]*sin(hitPhi[ih]),hitTime[ih]); 
      }
   }
   ...
   c1->SaveAs(Form("cluSignalView_%i.pdf",entry));
