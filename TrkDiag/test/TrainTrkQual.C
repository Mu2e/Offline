...
int
TrainTrkQual(TTree* mytree,int bkgw=exponential,const char* tname = "TrkQual")
{
...
// signal is defined as the momentum resolution core,
  TCut signal("de.status>0&&demcent.mom>100&&demcent.td>0.57&&demcent.td<1.0&&de.mom-demcent.mom<0.25&&de.mom-demcent.mom>-0.25");
  // tail is defined as the high-side tail
  TCut bkg("de.status>0&&demcent.mom>100&&demcent.td>0.57&&demcent.td<1.0&&de.mom-demcent.mom>0.7");
...
//  dataloader->AddVariable("lastflt-firstflt","FltLen","mm",'F');
  ...
  dataloader->PrepareTrainingAndTestTree(signal, bkg, "SplitMode=Random:!V:SplitSeed=89281:nTrain_Signal=40000" );
  ...
  // dataloader->OptimizeAllMethods("SigEffAt001","Scan");
  // dataloader->OptimizeAllMethods("ROCIntegral","GA");

  // --------------------------------------------------------------------------------------------------

  // ---- Now you can tell the factory to train, test, and evaluate the MVAs

//  dataloader->PrintHelpMessage();
//  dataloader->EvaluateAllVariables();
   factory->SetVerbose();
