...
// Fixed size dimensions of array or collections stored in the TTree if any.
 ...
 //mu2e::RobustHelix *mch_;
   ...
   TrkRecoEff(TTree *tree,double norm=-1.0);
   virtual ~TrkRecoEff();
...
#ifdef TrkRecoEff_cxx
TrkRecoEff::~TrkRecoEff()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t TrkRecoEff::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t TrkRecoEff::LoadTree(Long64_t entry)
{
...
void TrkRecoEff::Init(TTree *tree)
{
...
void TrkRecoEff::Show(Long64_t entry)
{
