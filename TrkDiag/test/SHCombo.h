...
// Fixed size dimensions of array or collections stored in the TTree if any.
 ...
 //CLHEP::Hep3Vector *mcgpos_;
   ...
   SHCombo(TTree* tree, float maxdwp=4.0, float maxdt=40.0,int maxds = 5);
   virtual ~SHCombo();
   ...
   int _maxds;
   float _maxdt, _maxdwp;

   std::vector<Int_t> _plane, _panel, _straw;
   std::vector<Int_t> _mcid;
   std::vector<float> _wd;
...
#ifdef SHCombo_cxx
SHCombo::SHCombo(TTree* tree, float maxdwp, float maxdt, int maxds):
  _maxds(maxds), _maxdt(maxdt), _maxdwp(maxdwp)
{
...
Int_t SHCombo::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t SHCombo::LoadTree(Long64_t entry)
{
...
void SHCombo::Show(Long64_t entry)
{
