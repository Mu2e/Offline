...
CreateMVAFactory(TTree* inputTree,
  ...
  TCut const& sigCut, TCut const& bkgCut,
  TFile* outputFile, TString const& sigweight, TString const& bkgweight)
{
...
//  }
  ...
  TMVA::DataLoader *dataloader = new TMVA::DataLoader("TrainBkgMVA");
  ...
  */
  ...
  if(sigweight!=""){
    std::cout << "Signal weight expression = " << sigweight << std::endl;
    dataloader->SetSignalWeightExpression    (sigweight);
  }
  if(bkgweight!=""){
    std::cout << "Signal weight expression = " << bkgweight << std::endl;
    dataloader->SetBackgroundWeightExpression(bkgweight);
  }

  dataloader->PrepareTrainingAndTestTree(sigCut, bkgCut,"SplitMode=Random:!V:SplitSeed=592309" ); 
...
//selection cuts
  ...
  vardescrip.push_back("Number of Planes");
...
// create the factory
  ...
  std::cout << "Defining signal with cut: " << sigCut.GetTitle() << std::endl;
  std::cout << "Defining background with cut: " << bkgCut.GetTitle() << std::endl;
  TMVA::Factory* factory = CreateMVAFactory(mytree,varnames, vardescrip,
  sigCut,bkgCut,rootFile, sigweight, bkgweight);
// run the training
