...
ebins = int(sys.argv[2])
ewidth = float(sys.argv[3])
tbins = int(sys.argv[4])
twidth = float(sys.argv[5])
...
t.Project("htn","totcal*(tcal < thv)+tothv*(tcal >= thv)","(ctime-mcsptime+pbtmc-ptime)*(mcsptime < 1800 && mcproc == 56)")
...
t.Project("hn","edep:totcal*(tcal < thv)+tothv*(tcal >= thv)","(ctime-mcsptime+pbtmc-ptime)*(mcsptime < 1800 && mcproc == 56)")
t.Project("hd","edep:totcal*(tcal < thv)+tothv*(tcal >= thv)","(mcsptime < 1800 && mcproc == 56)")
hn.Divide(hd)
...
for i in range(t.GetEntries()):
  if i > 100000:
    break
  t.GetEntry(i)
  if t.mcsptime > 1800 or t.mcproc != 56:
    continue
