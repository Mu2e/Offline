// Plot the r,phi,time coordinates of all clusters (black = bkg, red = CE), and plot the CE that fail the mva cut (MVA>0.5) (bottom right) 
//
...
void docluAnaSignalMVA(int entry)
{
   ...
   c1->SaveAs(Form("cluSignalMVA_%i.pdf",entry));
...
void cluAnaSignalMVA(int entry =-1){
   ...
   for (int i=0;i<100;++i) _hview4[i]  = new TH2F(Form("hview4_%i",i),      " ",           200, 400, 720, 100, -3.15, 3.15);
   ...
   if (entry==-1) for (int i=1;i<50;++i) docluAnaSignalMVA(i);
   else docluAnaSignalMVA(entry);
