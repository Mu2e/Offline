...
namespace mu2e {
  ...
  void PrimaryAntiProtonGun::produce(art::Event& event )
  {
    ...
    for (const auto& simPartPair: simParticles) {
      ...
      //
      // I want the proton at its first protonInelastic.  Eventually I
      // will weight these events according to the measured total
      // cross-section for pbar production so I can just take this. It
      // could be wrong if the proton has an inelastic collision and
      // then survives for another inelastic collision that produces a
      // pbar.
      //
      // let's see how often this happens.  Note for some reason
      // ProtonInelastic and protonInelastic are both process codes...
    ...
    //
    // and now 1/2 the real work: for the proton with the
    // protonInelastic stopping code, create a new SimParticle with
    // this proton information

    // first, we need to know the energy of the proton so we can set a
    // limit for the energy of the antiproton
    ...
    //
    // need kinematic max for pbar in the Lab frame.  all the
    // parameterizations we use assume a free proton at rest, even
    // though the target is something else. Assume that for
    // consistency.
    //
    // the way I calculate the max energy in the Lab frame is:
    // 1) assume two protons smashing together along the z axis; one
    // is our beam proton, which may have lost some energy before
    // interacting, one the target at rest
    // 2) calculate the maximum pbar energy for that beam proton
    // 3) boost back to lab.  I only care about the magnitude of the
    // maximum momentum for this calculation, so none of the
    // directions matter!
    ...
    //
    //these quantities belong to the antiproton; you can look these up
    //(one of many places: F. Taylor et al., PRD14,No. 5, 1 Sept 1976
    //p. 1217)
    ...
    //
    // take the biggest possible antiproton energy in the CM and boost
    // it back to the Lab. betaCM sign is defined by CLHEP, ultimately
    // same as ROOT.
    ...
    //
    // next stage will fail unless you tell it this new particle is
    // the daughter of the proton!
