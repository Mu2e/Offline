...
namespace mu2e {
    ...
    , doHistograms_       (pset.get<bool>("doHistograms",true ) )
  {
    ...
    if ( doHistograms_ ) {
      ...
      //_hEnergyPositron = tfdir.make<TH1F>("hEnergy", "Energy", 1200,  0.07,  120.07);
      _hEnergy = tfdir.make<TH1F>("hEnergy", "Energy", 2400,  0.0,  120.0);
                                                 ...
                                                 double *ehi)
  {
    ...
    if (spectrumShape == "Czarnecki") {
      ...
      *ehi         = (ratio+1.)*bin;
      ...
      //Spectrum.initialize<ConversionSpectrum>(*eMax,*elow,*ehi,bin,*eMax,bin,*ehi);
      //res.initialize<ConversionSpectrum>(*eMax,*elow,*ehi,bin,*eMax,bin,*ehi);
      /// looks crazy, but it will work
      //  Spectrum.initialize<ConversionSpectrum>(*ehi,*elow,*eMax,bin,*ehi,bin);

      // res.initialize<ConversionSpectrum>(*ehi,*elow,*eMax,bin,*ehi,bin);
