...
#### for adjacentPulseTimeDifference = 5ns
        ...
        generate_fcl --description=$name --dsconf=$i --run=1 --events=100000 --njobs=10 $fclfile
        ls $PWD/000/cnf.$USER.$name.$i.*.fcl > $fcllist
        clustername=CRV_efficiency5cm_top_moduleGap$moduleGap'_'layerOffset$layerOffset'_'photonYield$photonYield
...
#        generate_fcl --description=$name --dsconf=$i --run=1 --events=100000 --njobs=10 $fclfile
#        ls $PWD/000/cnf.$USER.$name.$i.*.fcl > $fcllist
#        clustername=CRV_efficiency5cm10_top_moduleGap$moduleGap'_'layerOffset$layerOffset'_'photonYield$photonYield
#        mu2eprodsys --setup=./setup.sh --fcllist=$fcllist --clustername=$clustername --dsconf=$i --wfproject=$name
