...
G4OpWLSPOPOP::G4OpWLSPOPOP(const G4String& processName, G4ProcessType type)
  : G4VDiscreteProcess(processName, type)
{
  SetProcessSubType(fOpWLS);   //TODO: There isn't a process name for OpWLSPOPOP
...
//
G4VParticleChange*
G4OpWLSPOPOP::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
{
  ...
  const G4MaterialPropertyVector* WLS_Intensity = 
    aMaterialPropertiesTable->GetProperty("WLSPOPOPCOMPONENT"); 
  ...
  const G4MaterialPropertyVector* WLS_QuantumYield = 
    aMaterialPropertiesTable->GetProperty("WLSPOPOPQUANTUMYIELD");
  ...
  WLSTime   = aMaterialPropertiesTable->
    GetConstProperty("WLSPOPOPTIMECONSTANT");
  WLSIntegral =
  ...
  if (verboseLevel>0) {
    G4cout << "\n Exiting from G4OpWLSPOPOP::DoIt -- NumberOfSecondaries = " 
...
void G4OpWLSPOPOP::BuildPhysicsTable(const G4ParticleDefinition&)
{
  if (theIntegralTable) {
     theIntegralTable->clearAndDestroy();
     delete theIntegralTable;
...
// ---------------
//
G4double G4OpWLSPOPOP::GetMeanFreePath(const G4Track& aTrack,
 				         G4double ,
				         G4ForceCondition* )
{
  const G4DynamicParticle* aParticle = aTrack.GetDynamicParticle();
...
void G4OpWLSPOPOP::UseTimeProfile(const G4String name)
{
  if (name == "delta")
    {
      delete WLSTimeGeneratorProfile;
    ...
    {
      G4Exception("G4OpWLSPOPOP::UseTimeProfile", "em0202",
                  FatalException,
