...
G4OpWLSY11::G4OpWLSY11(const G4String& processName, G4ProcessType type)
  : G4VDiscreteProcess(processName, type)
{
  SetProcessSubType(fOpWLS);   //TODO: There isn't a process name for OpWLSY11
...
//
G4VParticleChange*
G4OpWLSY11::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
{
  ...
  const G4MaterialPropertyVector* WLS_Intensity = 
    aMaterialPropertiesTable->GetProperty("WLSY11COMPONENT"); 
  ...
  const G4MaterialPropertyVector* WLS_QuantumYield = 
    aMaterialPropertiesTable->GetProperty("WLSY11QUANTUMYIELD");
  ...
  WLSTime   = aMaterialPropertiesTable->
    GetConstProperty("WLSY11TIMECONSTANT");
  WLSIntegral =
  ...
  if (verboseLevel>0) {
    G4cout << "\n Exiting from G4OpWLSY11::DoIt -- NumberOfSecondaries = " 
...
void G4OpWLSY11::BuildPhysicsTable(const G4ParticleDefinition&)
{
  if (theIntegralTable) {
     theIntegralTable->clearAndDestroy();
     delete theIntegralTable;
...
// ---------------
//
G4double G4OpWLSY11::GetMeanFreePath(const G4Track& aTrack,
 				         G4double ,
				         G4ForceCondition* )
{
  const G4DynamicParticle* aParticle = aTrack.GetDynamicParticle();
...
void G4OpWLSY11::UseTimeProfile(const G4String name)
{
  if (name == "delta")
    {
      delete WLSTimeGeneratorProfile;
    ...
    {
      G4Exception("G4OpWLSY11::UseTimeProfile", "em0202",
                  FatalException,
