...
PBARFTFModel::PBARFTFModel(const G4String& modelName):G4VPartonStringModel(modelName),
                         theExcitation(new PBARDiffractiveExcitation()),
...
// ------------------------------------------------------------
void PBARFTFModel::Init(const G4Nucleus & aNucleus, const G4DynamicParticle & aProjectile)
{
...
// ------------------------------------------------------------
G4ExcitedStringVector * PBARFTFModel::GetStrings()
{ 
...
/*yzy    
          for(G4int i=0; i < NumberOfInvolvedNucleon; i++ )
          {
            ...
            G4cout << "before BuildStrings aNucleon " << aNucleon->GetSplitableHadron()->GetDefinition()->GetParticleName()
                   << " energy " << aNucleon->GetSplitableHadron()->Get4Momentum().mag() << G4endl;
          }
*/
...
/* yzy
int NumberOfExcitedStrings = theStrings->size();
G4cout << "NumberOfExcitedStrings " << NumberOfExcitedStrings << G4endl;
for (int i = 0; i < NumberOfExcitedStrings; i++)
{
  G4cout << (*theStrings)[i]->Get4Momentum().mag() << G4endl;
}
*/
...
//-------------------------------------------------------------------
void PBARFTFModel::StoreInvolvedNucleon()                             
...
//-------------------------------------------------------------------
void PBARFTFModel::ReggeonCascade()                             
...
// changes in PBARDiffractiveExcitation.cc and G4ElasticHNScattering.cc
}                                                             // Uzhi 26 July 2009

// ------------------------------------------------------------
G4bool PBARFTFModel::PutOnMassShell()
{
	     ...
	     for(G4int i=0; i < NumberOfInvolvedNucleon; i++ )
             {
               ...
               aNucleon->SetMomentum(tmp);
//G4cout << "PutOnMassShell aNucleon " << aNucleon->GetDefinition()->GetParticleName()
//                   << " energy " << aNucleon->Get4Momentum().mag() << G4endl;
...
//G4cout << "Residual3 aNucleon " << aNucleon->GetDefinition()->GetParticleName()
//                   << " energy " << aNucleon->Get4Momentum().mag() << G4endl;
...
// ------------------------------------------------------------
G4bool PBARFTFModel::ExciteParticipants()
{
//G4cout<<"PBARFTFModel::ExciteParticipants() "<<G4endl;
...
/*yzy
         ...
         {
           G4Nucleon * aNucleon = TheInvolvedNucleon[i];
           G4cout << "17 aNucleon " << aNucleon->GetSplitableHadron()->GetDefinition()->GetParticleName()
                   << " energy " << aNucleon->GetSplitableHadron()->Get4Momentum().mag() << G4endl;
         }
G4cout<<"Before  pro "<<projectile->Get4Momentum()<<" "<<projectile->Get4Momentum().mag()<<G4endl;
G4cout<<"Before  tar "<<target->Get4Momentum()<<" "<<target->Get4Momentum().mag()<<G4endl;
...
/*yzy
         ...
         {
           G4Nucleon * aNucleon = TheInvolvedNucleon[i];
           G4cout << "20 aNucleon " << aNucleon->GetSplitableHadron()->GetDefinition()->GetParticleName()
                   << " energy " << aNucleon->GetSplitableHadron()->Get4Momentum().mag() << G4endl;
         }
*/
//Successfull=true;
//G4cout<<"PBARFTFModel::ExciteParticipants() Successfull "<<Successfull<<G4endl;
//G4int Uzhi; G4cin>>Uzhi;
	return Successfull;
}

//-------------------------------------------------------------------
void PBARFTFModel::AjustTargetNucleonForAnnihilation(G4VSplitableHadron *SelectedAntiBaryon,
...
// ------------------------------------------------------------
G4ExcitedStringVector * PBARFTFModel::BuildStrings()
{	
...
/*yzy
            G4cout << "hadron " << primaries[ahadron]->GetDefinition()->GetParticleName() 
                   << " energy " << primaries[ahadron]->Get4Momentum().mag() << G4endl;
*/
...
// Looking for spectator nucleons of the projectile-----------
	   ...
	   G4bool isProjectile=true;
           FirstString=0; SecondString=0;
/*yzy
           G4cout << "ProjectileSplitable " << ProjectileSplitable->GetDefinition()->GetParticleName()
                   << " energy " << ProjectileSplitable->Get4Momentum().mag() << G4endl;
*/
...
/*yzy
           G4cout << "theAdditionalString  " << theAdditionalString[ahadron]->GetDefinition()->GetParticleName()
                   << " energy " << theAdditionalString[ahadron]->Get4Momentum().mag() << G4endl;
*/
            FirstString=0; SecondString=0;
...
/*yzy 
             G4cout << "TheInvolvedNucleon  " << TheInvolvedNucleon[ahadron]->GetSplitableHadron()->GetDefinition()->GetParticleName()
                   << " energy " << TheInvolvedNucleon[ahadron]->GetSplitableHadron()->Get4Momentum().mag() << G4endl;
*/
...
// ------------------------------------------------------------
void PBARFTFModel::GetResidualNucleus()
...
// ------------------------------------------------------------
G4ThreeVector PBARFTFModel::GaussianPt(G4double AveragePt2, G4double maxPtSquare) const
...
void PBARFTFModel::ModelDescription(std::ostream& desc) const
{
