...
// ---------------------------------------------------------------------
G4bool PBARDiffractiveExcitation::
...
/*yzy
G4cout << "1 projectileE " << Pprojectile.mag() << " targetE " << Ptarget.mag() << G4endl;
*/
// ----------------------------------------------------------
...
/*yzy
G4cout << "2 projectile " << projectile->Get4Momentum().mag() << " target " << target->Get4Momentum().mag() << G4endl;
*/
        G4bool Result= theElastic->ElasticScattering (projectile,target,theParameters);
        return Result;
...
//G4cout << "ProbOfDiffraction " << ProbOfDiffraction << G4endl;
...
//	   	 G4cout << "PBARDiffractiveExcitation::ExciteParticipants possibly looping"
...
/*yzy
G4cout << "(Pprojectile+Qmomentum) " << (Pprojectile+Qmomentum).mag() << " ProjectileDiffStateMinMass " << std::sqrt(ProjectileDiffStateMinMass2) << G4endl;
*/
...
*/
/*yzy
G4cout << "(Ptarget-Qmomentum) " << (Ptarget-Qmomentum).mag() << " TargetDiffStateMinMass " << std::sqrt(TargetDiffStateMinMass2) << G4endl;
*/
...
/*yzy
G4cout << "3 projectile " << projectile->Get4Momentum().mag() << " target " << target->Get4Momentum().mag() << G4endl;
G4cout << "Qmomentum " << Qmomentum << G4endl;
*/
        }
...
/*yzy
//G4cout << "3.5 projectileE " << Pprojectile.mag() << " targetE " << Ptarget.mag() << " Qmomentum " << Qmomentum.mag() << G4endl;
...
*/
     ...
     Ptarget     -= Qmomentum;
/*yzy
G4cout << "3.6 whilecount " << whilecount << " projectileE " << Pprojectile.mag() << " targetE " << Ptarget.mag() << " Qmomentum " << Qmomentum.mag() << G4endl;
G4cout << "Pprojectile " << Pprojectile << G4endl;
G4cout << "Ptarget " << Ptarget << G4endl;
*/
//G4cout<<"Pr Y "<<Pprojectile.rapidity()<<" Tr Y "<<Ptarget.rapidity()<<G4endl;
...
///*yzy
//G4cout << "4 projectile " << projectile->Get4Momentum().mag() << " target " << target->Get4Momentum().mag() << G4endl;
//*/
     return true;
}

// ---------------------------------------------------------------------
void PBARDiffractiveExcitation::CreateStrings(G4VSplitableHadron * hadron, 
                                            G4bool isProjectile,
...
// ---------------------------------------------------------------------
G4double PBARDiffractiveExcitation::ChooseP(G4double Pmin, G4double Pmax) const
{
...
//  to be improved...
	...
	if ( Pmin <= 0. || range <=0. )
	{
		...
		throw G4HadronicException(__FILE__, __LINE__, "PBARDiffractiveExcitation::ChooseP : Invalid arguments ");
	}
...
// ---------------------------------------------------------------------
G4ThreeVector PBARDiffractiveExcitation::GaussianPt(G4double AveragePt2, 
...
// ---------------------------------------------------------------------
G4double PBARDiffractiveExcitation::GetQuarkFractionOfKink(G4double zmin, G4double zmax) const
    {
       G4double z, yf;
       do {
...
// ---------------------------------------------------------------------
void PBARDiffractiveExcitation::UnpackMeson(const G4int IdPDG, G4int &Q1, G4int &Q2) const // Uzhi 7.09.09
    {
...
// ---------------------------------------------------------------------
void PBARDiffractiveExcitation::UnpackBaryon(G4int IdPDG, 
...
// ---------------------------------------------------------------------
G4int PBARDiffractiveExcitation::NewNucleonId(G4int Q1, G4int Q2, G4int Q3) const // Uzhi 7.09.09
    {
       G4int TmpQ(0);
       if( Q3 > Q2 ) 
       {
...
// ---------------------------------------------------------------------
PBARDiffractiveExcitation::PBARDiffractiveExcitation(const PBARDiffractiveExcitation &)
{
	throw G4HadronicException(__FILE__, __LINE__, "PBARDiffractiveExcitation copy contructor not meant to be called");
}
...
const PBARDiffractiveExcitation & PBARDiffractiveExcitation::operator=(const PBARDiffractiveExcitation &)
{
...
int PBARDiffractiveExcitation::operator==(const PBARDiffractiveExcitation &) const
{
	throw G4HadronicException(__FILE__, __LINE__, "PBARDiffractiveExcitation == operator meant to be called");
	return false;
}
