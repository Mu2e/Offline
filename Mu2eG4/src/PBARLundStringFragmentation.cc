...
PBARLundStringFragmentation::PBARLundStringFragmentation()
{
G4cout << "Using PBARLundString for pbar enhancement" << G4endl;
...
//---------------------------------------
   ...
   BaryonWeight[0][1][2][1]=(1.-pspin_barion)*0.5;// 9.6 patch 
...
//---------------------------------------
   ...
   BaryonWeight[2][1][2][0]=1.-pspin_barion;
         ...
         BaryonWeight[2][1][2][1]=pspin_barion; // 9.6 patch
...
//--------------------------------------------------------------------------------------
void PBARLundStringFragmentation::SetMinimalStringMass(const G4FragmentingString  * const string)  
{
	...
	//G4cout<<"Min mass Qright "<<Qright<<" "<<EstimatedMass<<G4endl;
        //G4cout<<"Number_of_quarks "<<Number_of_quarks<<G4endl;
...
//--------------------------------------------------------------------------------------
void PBARLundStringFragmentation::SetMinimalStringMass2(
...
//--------------------------------------------------------------------------------------
G4KineticTrackVector* PBARLundStringFragmentation::FragmentString(
		const G4ExcitedString& theString)
{
//G4cout << "Begin FragmentString mass " << theString.Get4Momentum().mag() << G4endl;
...
/*yzy
if (Hadron->GetDefinition()->GetPDGEncoding() == -2212 || Hadron->GetDefinition()->GetPDGEncoding() == -2212 )
  G4cout << "Found pbar in 0 " << G4endl;
*/
	};

/*yzy
G4cout << "End of FragmentString mass " << theString.Get4Momentum().mag() << G4endl;
*/
	return LeftVector;
}

//----------------------------------------------------------------------------------
G4bool PBARLundStringFragmentation::IsFragmentable(const G4FragmentingString * const string)
{
...
//----------------------------------------------------------------------------------------
G4bool PBARLundStringFragmentation::StopFragmenting(const G4FragmentingString * const string)
{
	SetMinimalStringMass(string);
//G4cout << "StopFragmenting string Mass " << string->Mass() << " MinimalStringMass " << MinimalStringMass << " ratio " << std::exp(-0.0005*(string->Mass() - MinimalStringMass)) << G4endl;
...
//----------------------------------------------------------------------------------------------------------
G4bool PBARLundStringFragmentation::SplitLast(G4FragmentingString * string,
...
/*yzy
if (LeftHadron->GetPDGEncoding() == -2212 || RightHadron->GetPDGEncoding() == -2212 )
  G4cout << "In SplitLast FourQuarkString()" << G4endl;
*/
        } else
	{
...
/*yzy
if (LeftHadron->GetPDGEncoding() == -2212 || RightHadron->GetPDGEncoding() == -2212 )
  G4cout << "In SplitLast Quark_AntiQuark_lastSplitting()" << G4endl;
*/
		} else 
...
/*yzy
if (LeftHadron->GetPDGEncoding() == -2212 || RightHadron->GetPDGEncoding() == -2212 )
  G4cout << "In SplitLast Quark_Diquark_lastSplitting()" << G4endl;
*/
		}
...
//----------------------------------------------------------------------------------------------------------
void PBARLundStringFragmentation::Sample4Momentum(G4LorentzVector* Mom, G4double Mass, G4LorentzVector* AntiMom, G4double AntiMass, G4double InitialMass) 
{
...
//-----------------------------------------------------------------------------
G4LorentzVector * PBARLundStringFragmentation::SplitEandP(G4ParticleDefinition * pHadron,
...
//-----------------------------------------------------------------------------------------
G4double PBARLundStringFragmentation::GetLightConeZ(G4double zmin, G4double zmax, 
...
//------------------------------------------------------------------------
G4double PBARLundStringFragmentation::lambda(G4double S, G4double m1_Sqr, G4double m2_Sqr)
{ 
...
//----------------------------------------------------------------------------------------
G4bool PBARLundStringFragmentation::Loop_toFragmentString(G4ExcitedString * & theStringInCMS, 
                                                        G4KineticTrackVector * & LeftVector, 
                                                        G4KineticTrackVector * & RightVector)
{
...
//*yzy		//G4cout<<"Main loop start whilecounter "<<attempt<<G4endl;
		...
		{       // Split current string into hadron + new string
			...
			{
//*yzy				//G4cout<<"Hadron prod at fragm. "<<Hadron->GetDefinition()->GetParticleName()<<G4endl;
		...
		{
//*yzy
                  bool has_antiproton = false;
                
                  //G4cout << "LeftVector size " << LeftVector->size() << G4endl;
                  for (int i = 0; i < (int)LeftVector->size(); i++)
                  {
                    //G4cout << (*LeftVector)[i]->GetDefinition()->GetParticleName() << "  "
                      //     << (*LeftVector)[i]->Get4Momentum()
                        //  << G4endl;
                    G4int pdg = (*LeftVector)[i]->GetDefinition()->GetPDGEncoding();
                    if (pdg == -2212 || pdg == -2112 ||
                      pdg == -3112 || pdg == -3122 || pdg == -3212 || pdg == -3222 || pdg == -3312 || pdg == -3322) 
                    {
                      has_antiproton = true;
//                      G4cout << "anti_proton " << pdg << " generated on left  end, stop whilecounter at " << attempt << G4endl; 
                    }
                  }
                  //G4cout << "RightVector size " << RightVector->size() << G4endl;
                  for (int i = 0; i < (int)RightVector->size(); i++)
                  {
                    //G4cout << (*RightVector)[i]->GetDefinition()->GetParticleName() << "  "
                      //     << (*RightVector)[i]->Get4Momentum()
                        //   << G4endl;
                    G4int pdg = (*RightVector)[i]->GetDefinition()->GetPDGEncoding();
                    if (pdg == -2212 || pdg == -2112 ||
                      pdg == -3112 || pdg == -3122 || pdg == -3212 || pdg == -3222 || pdg == -3312 || pdg == -3322)
                    {
                      has_antiproton = true;
//                      G4cout << "anti_proton " << pdg << " generated on right end, stop whilecounter at " << attempt << G4endl;
                    }
                  }

                  if (has_antiproton) final_success=true;
		}
		delete currentString;
	}  // End of the loop where we try to fragment the string.

//        G4cout << "attemp=" << attempt << " final_success=" << final_success << G4endl;
	return final_success;
}

//----------------------------------------------------------------------------------------
G4bool PBARLundStringFragmentation::
...
/*yzy
if (LeftHadron->GetPDGEncoding() == -2212 || RightHadron->GetPDGEncoding() == -2212 )
  G4cout << "Found pbar in 4 " << G4endl;
*/
	return true;
}

//----------------------------------------------------------------------------------------------------------
G4int PBARLundStringFragmentation::SampleState(void) 
{
