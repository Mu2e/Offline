...
// $Id: DIOFilter_module.cc,v 1.1 2011/12/28 14:53:23 rhbob Exp $
// $Author: rhbob $
// $Date: 2011/12/28 14:53:23 $
//
...
namespace mu2e {
  ...
  DIOFilter1::filter(art::Event& event) {
    ...
    //
    // demand more than 15 straw hits and a hit crystal as part of the filter. Will distort momentum distribution but will make
    // files reasonably sized
    if (strawHits.size() < 15 || crystalHits.size() < 1){
      return false;
    }
