...
// $Id: ReasonableTrack_module.cc,v 1.1 2012/01/10 21:43:16 rhbob Exp $
// $Author: rhbob $
// $Date: 2012/01/10 21:43:16 $
//
...
namespace mu2e {
  ...
  class DIOFilter1 : public art::EDFilter {
  public:
    explicit DIOFilter1(fhicl::ParameterSet const& pset);
    virtual ~DIOFilter1() { }
  ...
  bool DIOFilter1::beginRun(art::Run& ){
  ...
  bool
  DIOFilter1::filter(art::Event& event) {
    ...
    //
    // demand more than 20 straw hits and a hit crystal as part of the filter. Will distort momentum distribution but will make
    // files reasonably sized
    if (strawHits.size() < 20  || crystalHits.size() < 1){
      return false;
    }
  ...
  void DIOFilter1::endJob() {
    mf::LogInfo("Summary") 
      << "DIOFilter1_module: Number of events passing the filter: " 
      << nPassed_
      << "\n";
  }
...
using mu2e::DIOFilter1;
DEFINE_ART_MODULE(DIOFilter1);
