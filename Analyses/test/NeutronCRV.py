...
process = mu2e.Process("NeutronRatesCounter")
...
process.TFileService = mu2e.Service("TFileService",
                       fileName = mu2e.string("NeutronRates.root"),
...
#       inputfile=mu2e.untracked.string("Mu2eG4/test/geom_01.txt")
       inputfile=mu2e.untracked.string("Analyses/test/geomCRV.txt")
)
...
# since looking at CRV, don't want this filter--> if no tracker or calo hits,still interesting
...
process.outfile = mu2e.OutputModule(
    "PoolOutputModule",
    fileName = mu2e.untracked.string('file:NeutronCRVData.root'),
...
process.output = mu2e.EndPath(   process.generate*process.g4run*process.CountRates );
