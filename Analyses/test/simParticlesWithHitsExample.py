#
# $Id: simParticlesWithHitsExample.py,v 1.1 2010/11/24 01:06:15 kutschke Exp $
# $Author: kutschke $
...
process = mu2e.Process("StrawHitExample")

# Maximum number of events to do.
process.maxEvents = mu2e.untracked.PSet(
    input = mu2e.untracked.int32(50)
)
...
process.TFileService = mu2e.Service("TFileService",
                       fileName = mu2e.string("strawHitExample.root"),
...
process.generate = mu2e.EDProducer(
    "EventGenerator",
    inputfile = mu2e.untracked.string("Mu2eG4/test/multiDIO.txt"),
...
process.outfile = mu2e.OutputModule(
    "PoolOutputModule",
    fileName = mu2e.untracked.string('file:strawHitExample_data.root'),
