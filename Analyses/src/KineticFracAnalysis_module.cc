 //
// An EDAnalyzer module that writes out calorimter and tracker information so that it can be used to study combined track/calo information.  Largely based on Echenard's CaloExample_module.cc
...
namespace mu2e {
  ...
  class KineticFracAnalysis : public art::EDAnalyzer {
   ...
   // Name of the tracker StepPoint collection
    ...
    // rhb added
       float _trkt0[8192],_trkMom[8192], _trkd0[8192],_trkz0[8192],_trkOmega[8192],_trkPhi0[8192],_trkt0Err[8192];
 ...
 //rhb added
  ...
  KineticFracAnalysis::KineticFracAnalysis(fhicl::ParameterSet const& pset) :
    ...
    _trackerHitsModuleLabel(pset.get<string>("trackerHitsModuleLabel")),
    _trackerHitsInstanceLabel(pset.get<string>("trackerHitsInstanceLabel")),
  ...
  void KineticFracAnalysis::analyze(const art::Event& event) {
	 ...
	 int fitPart = krep.particleType().particleType();

	 if (_diagLevel > 3){
	   std::cout << "particle type = " << fitPart << std::endl;
	 }
	  ...
	  if (_diagLevel > 3){
	    std:: cout << "KineticFracAnalysis t0 = " << krep.t0().t0() << " and momentum = " << p0.mag() << std::endl;
