...
namespace mu2e 
{
  class CaloEventDisplays : public art::EDAnalyzer {
    public:
        ...
        fhicl::Atom<art::InputTag> calocrysTag{Name("CaloHitCollection"),Comment("cal reco crystal hit info")};
    ...
    private: 
      ...
      const CaloHitCollection*  _calcryhitcol;
  ...
  Calorimeter const & cal = *(GeomHandle<Calorimeter>());
    ...
    typedef std::list<CaloHit const*> CaloCrystalList;
    CaloCrystalList firstlist;
    for(size_t i =0; i < _ncrystalhits; i++){ 
      CaloHit const& hit =(*_calcryhitcol)[i];
      firstlist.push_back(&hit);
     }

    firstlist.sort([] (CaloHit const* lhs, CaloHit const* rhs) {return lhs->energyDep() > rhs->energyDep();} );
    ...
    for(int i=0;i<674;i++){
	  ...
	  float _crystalEdep = 0;
    for(auto const& hit : firstlist){
      //CaloHit const& hit =(*_calcryhitcol).at(i);//_calcryhitcol
      ...
      if(hit->energyDep()>0){
        ...
        cout<<i<<" size  "<<firstlist.size()<<" energy "<<_crystalEdep<<endl;
        }
        i++;
  	  }
      ...
      << "  Subrun: " << event.id().subRun()
      << "  Event: " << event.id().event()<<" Total Cluster E: "<<_crystalEdep<<".root";
  ...
  bool CaloEventDisplays::findData(const art::Event& evt){
    ...
    auto cryhit = evt.getValidHandle<CaloHitCollection>(_calocrysTag);
