...
namespace mu2e {
  ...
  public:
    double chi2time()  const {return _bestChi2TimeThisTrack;}
    double chi2pos()   const {return _bestChi2PosThisTrack;}
    double chi2Match() const {return _bestChi2ThisTrack;}

  private:
    double _bestChi2TimeThisTrack;
    double _bestChi2PosThisTrack;
    double _bestChi2ThisTrack;
  };
  ...
  void TrackCaloAssns::produce(art::Event& event) {
	...
	//	_bestChi2TimeThisTrack = minChi2Time;
	//_bestChi2PosThisTrack = minChi2Pos;
	//_bestChi2ThisTrack = minChi2;
	if (_diagLevel > 2)
	...
	if (_nMatch > 2*_nCluster*static_cast<int>(kreps.size()) )
	  { throw cet::exception("RANGE") << " _nMatch and _nCluster mismatch, from TrackCaloAssns. Call Bob ...nmatch and ncluster are " <<  _nMatch << " " << _nCluster << std::endl;}
    ...
    if (trkIntersect.size()  > 2*kreps.size()) // can't have more than two intersections per track
      { 
	throw cet::exception("RANGE") << " more than two intersections for track and calorimeter, from TrackCaloAssns. Call Bob or Bertrand..." << std::endl;
      }
	...
	if (tCosDip0 == 0)
	  { throw cet::exception("RANGE") << " cosDip = 0 for track, which makes no sense, from TrackCaloAssns...it's Bobs fault" << std::endl;}
	...
	;
      ...
      if (_diagLevel > 4)
	{
	  std:: cout << "TrackCaloAssns _nTrk, t0 = " << _nTrk << " " << krep.t0().t0() << " and momentum " << p0.mag() << std::endl;
	  std:: cout << "TrackCaloAssns fitstatus = " << krep.fitStatus().success() << std::endl;
	}
    ...
    if (event.getByLabel(_shLabel,strawhitsH)) {
      ...
      _shcol  = 0;
      printf(" >>> ERROR in TrackCaloAssns::findData: StrawHitCollection with label=%s not found.\n",
	     _shLabel.data());
    }
    ...
    if (event.getByLabel(_shpLabel,shposH)) {
      ...
      _shpcol = 0;
      printf(" >>> ERROR in TrackCaloAssns::findData: StrawHitPositionCollection with label=%s not found.\n",
	     _shpLabel.data());
    }
    ...
    if (event.getByLabel(_shfLabel,shflagH)) {
      ...
      _shfcol = 0;
      printf(" >>> ERROR in TrackCaloAssns::findData: StrawHitFlagCollection with label=%s not found.\n",
	     _shfLabel.data());
    }
