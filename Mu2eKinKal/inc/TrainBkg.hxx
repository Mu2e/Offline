//Code generated automatically by TMVA for Inference of Model file [TrainBkg.h5] at [Thu Dec 29 03:31:28 2022] 
...
std::vector<float> fTensor_dense3kernel0 = std::vector<float>(9);
float * tensor_dense3kernel0 = fTensor_dense3kernel0.data();
std::vector<float> fTensor_dense2bias0 = std::vector<float>(9);
float * tensor_dense2bias0 = fTensor_dense2bias0.data();
std::vector<float> fTensor_dense2kernel0 = std::vector<float>(81);
...
std::vector<float> fTensor_dense1kernel0 = std::vector<float>(81);
float * tensor_dense1kernel0 = fTensor_dense1kernel0.data();
std::vector<float> fTensor_densebias0 = std::vector<float>(9);
...
std::vector<float> fTensor_dense2Dense = std::vector<float>(9);
float * tensor_dense2Dense = fTensor_dense2Dense.data();
std::vector<float> fTensor_dense1Relu0 = std::vector<float>(9);
...
std::vector<float> fTensor_dense2Relu0 = std::vector<float>(9);
float * tensor_dense2Relu0 = fTensor_dense2Relu0.data();
std::vector<float> fTensor_dense1Dense = std::vector<float>(9);
float * tensor_dense1Dense = fTensor_dense1Dense.data();
std::vector<float> fTensor_denseRelu0 = std::vector<float>(9);
float * tensor_denseRelu0 = fTensor_denseRelu0.data();
std::vector<float> fTensor_denseDense = std::vector<float>(9);
...
Session(std::string filename ="") {
   ...
   if (length != 81) {
      std::string err_msg = "TMVA-SOFIE failed to read the correct tensor size; expected size is 81 , read " + std::to_string(length) ;
      throw std::runtime_error(err_msg);
    }
    for (int i =0; i < length; ++i) 
...
//--------- Gemm
   ...
   int op_0_lda = 8;
   int op_0_ldb = 9;
   std::copy(tensor_densebias0, tensor_densebias0 + 9, tensor_denseDense);
...
//--------- Gemm
   ...
   int op_2_lda = 9;
   int op_2_ldb = 9;
   std::copy(tensor_dense1bias0, tensor_dense1bias0 + 9, tensor_dense1Dense);
...
//--------- Gemm
   ...
   int op_4_lda = 9;
   int op_4_ldb = 9;
   std::copy(tensor_dense2bias0, tensor_dense2bias0 + 9, tensor_dense2Dense);
