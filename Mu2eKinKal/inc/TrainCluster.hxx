//Code generated automatically by TMVA for Inference of Model file [TrainCluster.h5] at [Mon Jan  2 21:02:17 2023] 

#ifndef TMVA_SOFIE_TRAINCLUSTER
#define TMVA_SOFIE_TRAINCLUSTER
...
namespace TMVA_SOFIE_TrainCluster{
namespace BLAS{
...
std::vector<float> fTensor_dense3kernel0 = std::vector<float>(10);
float * tensor_dense3kernel0 = fTensor_dense3kernel0.data();
std::vector<float> fTensor_dense2bias0 = std::vector<float>(10);
float * tensor_dense2bias0 = fTensor_dense2bias0.data();
std::vector<float> fTensor_dense2kernel0 = std::vector<float>(100);
float * tensor_dense2kernel0 = fTensor_dense2kernel0.data();
std::vector<float> fTensor_dense1bias0 = std::vector<float>(10);
float * tensor_dense1bias0 = fTensor_dense1bias0.data();
std::vector<float> fTensor_dense1kernel0 = std::vector<float>(100);
...
std::vector<float> fTensor_dense2Dense = std::vector<float>(10);
float * tensor_dense2Dense = fTensor_dense2Dense.data();
std::vector<float> fTensor_dense1Relu0 = std::vector<float>(10);
...
std::vector<float> fTensor_dense2Relu0 = std::vector<float>(10);
float * tensor_dense2Relu0 = fTensor_dense2Relu0.data();
std::vector<float> fTensor_dense1Dense = std::vector<float>(10);
float * tensor_dense1Dense = fTensor_dense1Dense.data();
std::vector<float> fTensor_denseRelu0 = std::vector<float>(10);
float * tensor_denseRelu0 = fTensor_denseRelu0.data();
std::vector<float> fTensor_denseDense = std::vector<float>(10);
float * tensor_denseDense = fTensor_denseDense.data();


Session(std::string filename ="") {
   if (filename.empty()) filename = "TrainCluster.dat";
   std::ifstream f;
   f.open(filename);
   if (!f.is_open()){
...
//--------- Gemm
   ...
   int op_0_lda = 5;
   int op_0_ldb = 10;
   std::copy(tensor_densebias0, tensor_densebias0 + 10, tensor_denseDense);
...
//--------- Gemm
   ...
   int op_2_lda = 10;
   int op_2_ldb = 10;
   std::copy(tensor_dense1bias0, tensor_dense1bias0 + 10, tensor_dense1Dense);
...
//--------- Gemm
   ...
   int op_4_lda = 10;
   int op_4_ldb = 10;
   std::copy(tensor_dense2bias0, tensor_dense2bias0 + 10, tensor_dense2Dense);
...
} //TMVA_SOFIE_TrainCluster

#endif  // TMVA_SOFIE_TRAINCLUSTER
