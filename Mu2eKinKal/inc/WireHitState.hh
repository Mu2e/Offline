...
namespace mu2e {
  ...
  struct WireHitState {
    ...
    enum TimeConstraint { none=0, TOTonly=1, driftonly=2, both=3}; // how to constrain time
...
// convenience functions
    ...
    bool constrainTOT() const { return tcon_ == both || tcon_ == TOTonly; }

    bool constrainDt() const { return tcon_ == both || tcon_ == driftonly; }
    ...
    static TimeConstraint timeConstraint(std::string const& tcon);
    static NullDistVar nullDistVar(std::string const& ndvar);
    static std::vector<std::string> StateNames_;
    static std::vector<std::string> TimeConstraintNames_;
