...
void FMVATools::FlatWeights::pack(layers_weights_t const& layers) {
  ...
  for (auto const& neurons : layers) {
    if (neurons.size() == 0)
      throw cet::exception("RECO") << "mu2e::FMVATools: XML initialization error: Layers must have neuron(s).\n";
    ...
    for (auto const& weights : neurons) {
      if (_dims.back() != weights.size())
        throw cet::exception("RECO") << "mu2e::FMVATools: XML initialization error: All neurons of each "
                                        "layer must have the same number of weights.\n";
...
void FMVATools::FlatWeights::pack(values_t const& weights, values_t const& norms) {
  if (weights.size() != norms.size())
    throw cet::exception("RECO") << "mu2e::FMVATools: XML initialization error: Invalid first layer params.\n";
...
void FMVATools::initMVA() {
  ...
  for (auto const& a : _mva_tools._wn) weights.push_back(a[0]);

  auto norms = values_t();

  for (auto const& a : _mva_tools._wnr2) norms.push_back(1.0 / a);
...
inline float sigmoid_approx(float x) {
  // max approx error < 0.0035
  if (x > 10.0f) return 1.0f;

  float y = 1 - 0.015625 * x;
  y *= y;
  y *= y;
  y *= y;
  y *= y;
