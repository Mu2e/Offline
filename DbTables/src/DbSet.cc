...
void DbSet::print() const {
  ...
  for (auto const& p : _emap) {
    int tid = p.first;
    for (auto r : p.second) {
      std::cout << std::setw(5) << tid << std::setw(20) << r.iov()
                << std::setw(8) << std::right << r.cid() << std::endl;
    }
  }
}
...
void DbSet::printShort() const {
  ...
  for (auto const& p : _emap) {
    std::cout << std::setw(5) << p.first << std::setw(8) << p.second.size()
              << std::endl;
  }
}
