...
class TrkThresholdRStraw : public DbTable {
 public:
  ...
  class Row {
   public:
    Row(int index, float threshold_hv, float threshold_cal) :
        _index(index), _threshold_hv(threshold_hv),
        _threshold_cal(threshold_cal) {}
  ...
  TrkThresholdRStraw() :
      DbTable(cxname, "trk.thresholdrstraw",
  ...
  void addRow(const std::vector<std::string>& columns) override {
    ...
    if (index != int(_rows.size())) {
      throw cet::exception("TRKTHRESHOLDRSTRAW_BAD_INDEX")
          << "TrkThresholdRStraw::addRow found index out of order: " << index
          << " != " << _rows.size() << "\n";
    }
