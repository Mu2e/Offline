...
class ValIovs : public DbTable {
 public:
  class Row {
   public:
    Row(int iid, int cid, int start_run, int start_subrun, int end_run,
        int end_subrun, std::string create_time, std::string create_user) :
        _iid(iid),
        _cid(cid), _start_run(start_run), _start_subrun(start_subrun),
        _end_run(end_run), _end_subrun(end_subrun), _create_time(create_time),
  ...
  ValIovs() :
      DbTable(cxname, "val.iovs",
              "iid,cid,start_run,start_subrun,end_run,end_subrun,create_time,"
              "create_user") {}
  ...
  size_t size() const override {
    return baseSize() + sizeof(this) + nrow() * nrow() / 2 + nrow() * 64;
  };
  ...
  void addRow(const std::vector<std::string>& columns) override {
    _rows.emplace_back(std::stoi(columns[0]), std::stoi(columns[1]),
                       ...
                       std::stoi(columns[4]), std::stoi(columns[5]), columns[6],
                       columns[7]);
