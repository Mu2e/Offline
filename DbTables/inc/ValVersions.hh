...
class ValVersions : public DbTable {
 public:
  class Row {
   public:
    Row(int vid, int pid, int lid, int major, int minor, std::string comment,
        std::string create_time, std::string create_user) :
        _vid(vid),
        _pid(pid), _lid(lid), _major(major), _minor(minor), _comment(comment),
  ...
  void addRow(const std::vector<std::string>& columns) override {
    _rows.emplace_back(std::stoi(columns[0]), std::stoi(columns[1]),
                       ...
                       std::stoi(columns[4]), columns[5], columns[6],
                       columns[7]);
