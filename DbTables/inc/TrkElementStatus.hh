...
class TrkElementStatus : public DbTable {
 public:
  ...
  struct TrkElementStatusRow {
    ...
    TrkElementStatusRow(StrawId const& sid, StrawStatus const& status) :
        _sid(sid), _status(status) {}
  };

  typedef std::shared_ptr<TrkElementStatus> ptr_t;
  typedef std::shared_ptr<const TrkElementStatus> cptr_t;

  TrkElementStatus(const char* Name, const char* DbName, StrawIdMask sidmask,
                   StrawStatus statusmask) :
      DbTable(Name, DbName, "strawid,strawstatus"),
      _sidmask(sidmask), _statusmask(statusmask) {}
  const TrkElementStatusRow& rowAt(const std::size_t index) const {
  ...
  size_t size() const override {
    return baseSize() + nrow() * sizeof(TrkElementStatusRow);
  }
  // table-specific info
  ...
  void addRow(const std::vector<std::string>& columns) override {
    ...
    if (!_statusmask.hasAllProperties(status))
      throw cet::exception(name())
          << "Illegal status specified " << status << std::endl;
...
// unique classes for Db usage.  Each defines a mask and for StrawId and the
// StrawStatus
class TrkPanelStatus : public TrkElementStatus {
 public:
  constexpr static const char* cxname = "TrkPanelStatus";
  TrkPanelStatus() :
      TrkElementStatus(cxname, "trk.panelstatus", StrawIdMask("uniquepanel"),
                       StrawStatus("Absent:NoHV:NoGas:NoLV:LowGasGain")) {}
};
...
class TrkPlaneStatus : public TrkElementStatus {
 public:
  ...
  TrkPlaneStatus() :
      TrkElementStatus(cxname, "trk.planestatus", StrawIdMask("plane"),
                       StrawStatus("Absent")) {}
};
// split individual straw status tables in 2: one for short-term, one for
// long-term
class TrkStrawStatusShort : public TrkElementStatus {
 public:
  constexpr static const char* cxname = "TrkStrawStatusShort";
  TrkStrawStatusShort() :
      TrkElementStatus(cxname, "trk.strawstatusshort",
                       StrawIdMask("uniquestraw"),
                       StrawStatus("Sparking:Suppress:Noise:Pickup:Disabled")) {
  }
};
class TrkStrawStatusLong : public TrkElementStatus {
 public:
  ...
  TrkStrawStatusLong() :
      TrkElementStatus(
          cxname, "trk.strawstatuslong", StrawIdMask("uniquestraw"),
          StrawStatus(
