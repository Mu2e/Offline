...
class DbVersion {
 public:
  DbVersion() :
      _purpose(std::string("none")), _major(-1), _minor(-1), _extension(-1) {}
  DbVersion(std::string const& purpose, int major, int minor, int extension) :
      _purpose(purpose), _major(major), _minor(minor), _extension(extension) {}
  ...
  std::string to_string(std::string const& sep = std::string("/")) const {
    return _purpose + " " + std::to_string(_major) + sep +
           std::to_string(_minor) + sep + std::to_string(_extension);
  }
