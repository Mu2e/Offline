#ifndef DbTables_TrkAlignTracker_hh
#define DbTables_TrkAlignTracker_hh
...
namespace mu2e {
  ...
  class TrkAlignTracker : public DbTable {
  public:

    typedef std::shared_ptr<TrkAlignTracker> ptr_t;
    typedef std::shared_ptr<const TrkAlignTracker> cptr_t;
    ...
    TrkAlignTracker():DbTable("TrkAlignTracker","trk.aligntracker",
	 "index,dx,dy,dz,rx,ry,rz") {}
    const Row& rowAt(const std::size_t index) const { return _rows.at(index);}
    std::vector<Row> const& rows() const {return _rows;}
    std::size_t nrow() const { return _rows.size(); };
    virtual std::size_t nrowFix() const { return 1; };
