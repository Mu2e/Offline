...
class TstCalib3 : public DbTable {
 public:
  class Row {
   public:
    Row(int channel, float v0, float v1, float v2, float v3, float v4, float v5,
  ...
  TstCalib3() :
      DbTable(cxname, "tst.calib3", "channel,v0,v1,v2,v3,v4,v5,v6,v7,v8,v9") {}
  ...
  void addRow(const std::vector<std::string>& columns) override {
    _rows.emplace_back(
        std::stoul(columns[0]), std::stof(columns[1]), std::stof(columns[2]),
        std::stof(columns[3]), std::stof(columns[4]), std::stof(columns[5]),
        std::stof(columns[6]), std::stof(columns[7]), std::stof(columns[8]),
        std::stof(columns[9]), std::stof(columns[10]));
