...
class TrkAlignParams {
 public:
  TrkAlignParams(int index, StrawId const& id, float dx, float dy, float dz,
      ...
      _index(index),
      _id(id), _dx(dx), _dy(dy), _dz(dz), _rx(rx), _ry(ry), _rz(rz),
      _transform(dx, dy, dz, rx, ry, rz) {}
  // the following should really use float, but the alignment classes want
  // double FIXME!
  TrkAlignParams(StrawId const& id, StrawIdMask::Level level, double dx,
                 double dy, double dz, double rx, double ry, double rz) :
      _id(id),
      _dx(dx), _dy(dy), _dz(dz), _rx(rx), _ry(ry), _rz(rz),
      _transform(dx, dy, dz, rx, ry, rz) {
    switch (level) {
 ...
 private:
  int _index;  // index is redundant with StrawId and the table row count but is
               // required by the Db interface
  StrawId _id;
  float _dx;
  float _dy;
  float _dz;
