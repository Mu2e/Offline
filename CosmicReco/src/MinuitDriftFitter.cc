...
FitResult DoFit(int _diag, CosmicTrackFinderData& trackdata, StrawResponse const& srep,
                const Tracker* tracker, double max_doca, unsigned int minChits, int MaxLogL,
                double _gaussTres, double maxTres)
{
    ...
    GaussianPDFFit fit(trackdata._tseed._straw_chits, srep, cosmictrack, constraint_means,
                       constraints, _gaussTres, 1, tracker);
    ...
    if (_diag > 1) {
        std::cout << "========= FIRST STAGE GAUSSIAN FIT ==========" << std::endl;
        for (size_t i = 0; i < FitResult.names.size(); i++) {
    ...
    for (size_t i = 0; i < trackdata._tseed._straw_chits.size(); i++) {
        double gauss_end_doca = fit.calculate_DOCA(
            trackdata._tseed._straw_chits[i], FitResult.bestfit[0], FitResult.bestfit[1],
            FitResult.bestfit[2], FitResult.bestfit[3], tracker);
        double gauss_end_time_residual = fit.TimeResidual(
            gauss_end_doca, srep, FitResult.bestfit[4], trackdata._tseed._straw_chits[i], tracker);
    ...
    if (cosmictrack.minuit_converged and passed_hits.size() > minChits) {
        ...
        // Set Limits as tracker dimensions: // TODO: put these back
        newmigrad.SetLimits((signed)0, -10500, 10500);
        newmigrad.SetLimits((signed)1, -5, 5);
        newmigrad.SetLimits((signed)2, -5020, 5020);
        ...
        FitResult.bestfiterrors = results.Errors();
    if (_diag > 1) {
        std::cout << "========= SECOND STAGE FULL FIT =============" << std::endl;
        for (size_t i = 0; i < FitResult.names.size(); i++) {
            std::cout << i << ": " << FitResult.names[i] << " : " << FitResult.bestfit[i] << " +- "
                      << FitResult.bestfiterrors[i] << std::endl;
			std::cout << passed_hits.size() << " hits" << std::endl;
        }
        std::cout << "=============================================" << std::endl;
    }
        ...
        for (size_t i = 0; i < passed_hits.size(); i++) {
            ...
            double start_time_residual = fit.TimeResidual(
                start_doca, srep, seed[4], trackdata._tseed._straw_chits[i], tracker);
            double end_doca =
                fit.calculate_DOCA(passed_hits[i], FitResult.bestfit[0], FitResult.bestfit[1],
                                   FitResult.bestfit[2], FitResult.bestfit[3], tracker);
            double ambig =
                fit.calculate_ambig(passed_hits[i], FitResult.bestfit[0], FitResult.bestfit[1],
                                    FitResult.bestfit[2], FitResult.bestfit[3], tracker);
