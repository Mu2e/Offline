...
public:
  struct Config {
    ...
    fhicl::OptionalAtom<art::InputTag> caloTag{fhicl::Name("caloTag"),
                                               fhicl::Comment("Calo intensity info Tag")};
    fhicl::OptionalAtom<art::InputTag> timeClusterTag{
        fhicl::Name("timeClusterTag"), fhicl::Comment("Time cluster intensity info Tag")};
    fhicl::OptionalAtom<art::InputTag> trackerTag{fhicl::Name("trackerTag"),
                                                  fhicl::Comment("Tracker intensity info Tag")};
    fhicl::OptionalAtom<art::InputTag> headerTag{fhicl::Name("headerTag"),
                                                 fhicl::Comment("Event header Tag")};
    fhicl::OptionalAtom<int> eventFreq{
        fhicl::Name("eventFreq"),
        fhicl::Comment(
            "Frequency of event data product writing, defaulting to sub-runs if not set")};
    fhicl::Atom<bool> passFirst{
        fhicl::Name("passFirst"),
        fhicl::Comment("Pass first event per sub-run to ensure the output file is prepared"),
        false};
    fhicl::Atom<int> duplications{fhicl::Name("duplications"),
                                  fhicl::Comment("Duplicate input data N times"), 0};
  };
...
LumiStreamFilter::LumiStreamFilter(const art::EDFilter::Table<Config>& config) :
    art::EDFilter{config}, _diagLevel(config().diagLevel()), _useCalo(config().caloTag(_caloTag)),
    _useTimeCluster(config().timeClusterTag(_timeClusterTag)),
    _useTracker(config().trackerTag(_trackerTag)), _useHeader(config().headerTag(_headerTag)),
    _useSubruns(!config().eventFreq(_eventFreq)), _passFirst(config().passFirst()),
    _duplications(config().duplications()), _outName(app_name), _eventCount(0),
    _firstInSubrun(true), _caloInfos(nullptr), _timeClusterInfos(nullptr), _trackerInfos(nullptr),
    _headers(nullptr) {
  std::replace(_outName.begin(), _outName.end(), '_',
               'U'); // protect against forbidden branch name characters
  ...
  if (_diagLevel > 0) {
    ...
    if (_useCalo)
      std::cout << " (" << _caloTag.encode().c_str() << ")";
    std::cout << std::endl;
    std::cout << " useTimeCluster = " << _useTimeCluster;
    if (_useTimeCluster)
      std::cout << " (" << _timeClusterTag.encode().c_str() << ")";
    std::cout << std::endl;
    std::cout << " useTracker = " << _useTracker;
    if (_useTracker)
      std::cout << " (" << _trackerTag.encode().c_str() << ")";
    std::cout << " useHeader = " << _useHeader;
    if (_useHeader)
      std::cout << " (" << _headerTag.encode().c_str() << ")";
    std::cout << std::endl;
  ...
  if (_useTimeCluster) {
    _timeClusterInfos =
        std::unique_ptr<mu2e::IntensityInfosTimeCluster>(new mu2e::IntensityInfosTimeCluster);
    if (!_useSubruns)
  ...
  if (_useTracker) {
    _trackerInfos =
        std::unique_ptr<mu2e::IntensityInfosTrackerHits>(new mu2e::IntensityInfosTrackerHits);
    if (!_useSubruns)
  ...
  if (_useSubruns)
    ...
    if (_caloInfos)
      _caloInfos->reserve(_eventFreq);
    if (_timeClusterInfos)
      _timeClusterInfos->reserve(_eventFreq);
    if (_trackerInfos)
      _trackerInfos->reserve(_eventFreq);
  }
}
...
bool LumiStreamFilter::endSubRun(art::SubRun& sr) {
  ...
  if (_diagLevel > 0)
    std::cout << "LumiStreamFilter::" << __func__ << ": Run:SubRun " << runNumber << ":"
              << subrunNumber << " app_name = " << app_name
              << ": Writing out the accumulated intensity info collections from " << _eventCount
              << " events\n";
  TLOG(TLVL_DEBUG + 5) << "LumiStreamFilter::" << __func__ << ": Run:SubRun " << runNumber << ":"
                       << subrunNumber << " app_name = " << app_name
  ...
  if (_useCalo)
    sr.put(std::move(_caloInfos), _outName, art::fullSubRun());
  if (_useTimeCluster)
    sr.put(std::move(_timeClusterInfos), _outName, art::fullSubRun());
  if (_useTracker)
    sr.put(std::move(_trackerInfos), _outName, art::fullSubRun());
  if (_useHeader)
    sr.put(std::move(_headers), _outName, art::fullSubRun());
...
bool LumiStreamFilter::filter(art::Event& event) {
  ...
  if (_diagLevel > 2)
    std::cout << "LumiStreamFilter::" << __func__ << ": Begin processing Event " << runNumber << ":"
              << subrunNumber << ":" << eventNumber << std::endl;
  //---------------------------------------
  // Retrieve the data
  ...
  if (_useCalo) {
    ...
    if (!event.getByLabel(_caloTag, caloH) || !caloH.product()) {
      std::cout << "LumiStreamFilter::" << __func__ << ": Event " << runNumber << ":"
                << subrunNumber << ":" << eventNumber << ": Calo intensity object not found!\n";
      _caloInfos->push_back(
          mu2e::IntensityInfoCalo()); // add an empty one to maintain the list alignment
    } else {
      const auto caloInfo = caloH.product();
      _caloInfos->push_back(mu2e::IntensityInfoCalo(*caloInfo));
      if (_diagLevel > 2)
        std::cout << "LumiStreamFilter::" << __func__ << ": Event " << runNumber << ":"
                  << subrunNumber << ":" << eventNumber << ": Retrieved calo information\n";
    }
  }
  ...
  if (_useTimeCluster) {
    ...
    if (!event.getByLabel(_timeClusterTag, timeClusterH) || !timeClusterH.product()) {
      ...
      _timeClusterInfos->push_back(
          mu2e::IntensityInfoTimeCluster()); // add an empty one to maintain the list alignment
    } else {
      const auto timeClusterInfo = timeClusterH.product();
      _timeClusterInfos->push_back(mu2e::IntensityInfoTimeCluster(*timeClusterInfo));
      if (_diagLevel > 2)
        std::cout << "LumiStreamFilter::" << __func__ << ": Event " << runNumber << ":"
                  << subrunNumber << ":" << eventNumber << ": Retrieved time cluster information\n";
    }
  }
  ...
  if (_useTracker) {
    ...
    if (!event.getByLabel(_trackerTag, trackerH) || !trackerH.product()) {
      std::cout << "LumiStreamFilter::" << __func__ << ": Event " << runNumber << ":"
                << subrunNumber << ":" << eventNumber << ": Tracker intensity object not found!\n";
      _trackerInfos->push_back(
          mu2e::IntensityInfoTrackerHits()); // add an empty one to maintain the list alignment
    } else {
      const auto trackerInfo = trackerH.product();
      _trackerInfos->push_back(mu2e::IntensityInfoTrackerHits(*trackerInfo));
      if (_diagLevel > 2)
        std::cout << "LumiStreamFilter::" << __func__ << ": Event " << runNumber << ":"
                  << subrunNumber << ":" << eventNumber << ": Retrieved tracker information\n";
    }
  }
  ...
  if (_useHeader) {
    ...
    if (!event.getByLabel(_headerTag, headerH) || !headerH.product()) {
      std::cout << "LumiStreamFilter::" << __func__ << ": Event " << runNumber << ":"
                << subrunNumber << ":" << eventNumber << ": Event Header not found!\n";
      ...
      if (_diagLevel > 2)
        std::cout << "LumiStreamFilter::" << __func__ << ": Event " << runNumber << ":"
                  << subrunNumber << ":" << eventNumber << ": Retrieved Event Header\n";
    }
    TLOG(TLVL_DEBUG + 10) << "Event " << runNumber << ":" << subrunNumber << ":" << eventNumber
  ...
  if (_duplications > 0) {
    for (int icopy = 0; icopy < _duplications; ++icopy) {
      if (_useCalo)
        _caloInfos->push_back(_caloInfos->back());
      if (_useTimeCluster)
        _timeClusterInfos->push_back(_timeClusterInfos->back());
      if (_useTracker)
        _trackerInfos->push_back(_trackerInfos->back());
      if (_useHeader)
        _headers->push_back(_headers->back());
    }
  }
  ...
  if (retval) {
    ...
    if (_useCalo)
      event.put(std::move(_caloInfos));
    if (_useTimeCluster)
      event.put(std::move(_timeClusterInfos));
    if (_useTracker)
      event.put(std::move(_trackerInfos));
    if (_useHeader)
      event.put(std::move(_headers));
  ...
  if (_diagLevel > 1)
    std::cout << "LumiStreamFilter::" << __func__ << ": Event " << runNumber << ":" << subrunNumber
              << ":" << eventNumber << ": Return value = " << retval
              << " for event count = " << _eventCount << " events and event freq " << _eventFreq
              << std::endl;
  return retval;
} // filter
