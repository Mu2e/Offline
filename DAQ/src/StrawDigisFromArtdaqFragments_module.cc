...
public:
  struct Config {
    fhicl::Atom<int> diagLevel{fhicl::Name("diagLevel"),
                               fhicl::Comment("diagnostic severity level, default = 0"), 0};
    fhicl::Atom<int> debugLevel{fhicl::Name("debugLevel"),
                                fhicl::Comment("debug level, default = 0"), 0};
    fhicl::Atom<bool> saveWaveforms{fhicl::Name("saveWaveforms"),
                                    fhicl::Comment("save StrawDigiADCWaveforms, default = true"),
                                    true};

    fhicl::Atom<bool> missingDTCHeaders{
        fhicl::Name("missingDTCHeaders"),
        fhicl::Comment("Whether data was (not) produced using DTC_Events; set to true to disable "
                       "skipping DTC-level header in deserialization sequence.")};
    ...
    struct GeographicTuple {
      fhicl::Atom<std::string> minnesota{fhicl::Name("minnesota"),
                                         fhicl::Comment("Minnesota # label")};
      fhicl::Atom<uint16_t> plane{fhicl::Name("plane"),
                                  fhicl::Comment("Geographic plane [DocDB-888]")};
      fhicl::Atom<uint16_t> panel{fhicl::Name("panel"),
    ...
    struct LogicalTuple {
      fhicl::Atom<uint16_t> dtc{fhicl::Name("dtc"), fhicl::Comment("DTC ID")};
      fhicl::Atom<uint16_t> link{fhicl::Name("link"), fhicl::Comment("Link #")};
      fhicl::Atom<std::string> minnesota{fhicl::Name("minnesota"),
                                         fhicl::Comment("Minnesota # label")};
    };
    ...
    fhicl::OptionalSequence<fhicl::Table<LogicalTuple>> channeling{
        fhicl::Name("channeling"), fhicl::Comment("Logical channeling of panels (optional)")};
  };
...
private:
  ...
  std::map<uint16_t, uint16_t>
      minnesota_map_;           // mapping from minnesota number to upper bits of StrawId
...
// ======================================================================
art::StrawDigisFromArtdaqFragments::StrawDigisFromArtdaqFragments(
    const art::EDProducer::Table<Config>& config) :
    art::EDProducer{config},
    diagLevel_(config().diagLevel()), debugLevel_(config().debugLevel()),
    saveWaveforms_(config().saveWaveforms()), roc_payload_offset_(0) {
  ...
  for (auto handle : fragmentHandles) {
    ...
    if (handle->front().type() == artdaq::Fragment::ContainerFragmentType) {
      ...
      for (int ifrag = 0; ifrag < nfrag; ifrag++) {
        ...
        //-----------------------------------------------------------------------------
        DTCLib::DTC_SubEventHeader* seh =
            (DTCLib::DTC_SubEventHeader*)(fdata + roc_payload_offset_);
        ...
        while (roc_data < last_address) {
          ...
          if (rdh->packetCount > 1) {
            ...
            for (int ihit = 0; ihit < nhits; ihit++) {
              ...
              //-----------------------------------------------------------------------------
              // at this point, check consistency between the channel_id, dtc_id and link_id for a
              // given run panel ID is a derivative of the DTC ID and the link iD mn_id -
              // 'MinnesotaID' of the panel
              //-----------------------------------------------------------------------------
              ...
              //-----------------------------------------------------------------------------
                ...
                if (minnesota_map_.count(mn_id) < 1) {
                  std::string msg =
                      "undefined minnesota number in fallback mapping:" + std::to_string(mn_id);
                  throw cet::exception("StrawDigisFromArtdaqFragments") << msg << std::endl;
                }
                panel_id = minnesota_map_[mn_id];
              ...
              if (hit_data->NumADCPackets != nADCPackets_) {
                ...
                print_(
                    std::format("ERROR: wrong NADCpackets:{} , expected:{}, STOP PROCESSING HITS",
                                np, nADCPackets_));
                break;
              }
              //-----------------------------------------------------------------------------
              ...
              print_(std::format("offset:0x{:04x} sid:{:5} times: {:9} {:9} TOT:{:2}:{:2} pmp:{}",
                                 offset, straw_index, hit_data->TDC0(), hit_data->TDC1(), tot[0],
                                 tot[1], pmp),
                     1);
              ...
              if (saveWaveforms_) {
                ...
                auto idx = 2;
                auto adc_packet =
                    (mu2e::TrackerDataDecoder::TrackerADCPacket*)((char*)hit_data +
                                                                  16); // the packet size is 16
                                                                       // bytes
