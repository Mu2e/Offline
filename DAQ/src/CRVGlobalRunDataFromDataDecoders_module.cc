// CRVGlobalRunDataFromDataDecoders:  Print out CRV global run data
...
class CRVGlobalRunDataFromDataDecoders : public art::EDProducer
{
  public:
  struct Config
  {
  ...
  explicit CRVGlobalRunDataFromDataDecoders(const art::EDProducer::Table<Config>& config);
  ~CRVGlobalRunDataFromDataDecoders() override;
...
CRVGlobalRunDataFromDataDecoders::CRVGlobalRunDataFromDataDecoders(const art::EDProducer::Table<Config>& config) :
...
CRVGlobalRunDataFromDataDecoders::~CRVGlobalRunDataFromDataDecoders()
{
  if(_writeCsvFile)
  {
    _csvFile.close();
...
void CRVGlobalRunDataFromDataDecoders::produce(art::Event& event)
{
...
using mu2e::CRVGlobalRunDataFromDataDecoders;
DEFINE_ART_MODULE(CRVGlobalRunDataFromDataDecoders)
