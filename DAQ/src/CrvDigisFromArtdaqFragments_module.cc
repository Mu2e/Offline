...
public:
  struct Config {
    ...
    fhicl::Atom<bool> useSubsystem0{fhicl::Name("useSubsystem0"),
                                    fhicl::Comment("consider subevents encoded with subsystem 0")};
  };
...
CrvDigisFromArtdaqFragments::CrvDigisFromArtdaqFragments(
    const art::EDProducer::Table<Config>& config) :
    art::EDProducer{config},
    _diagLevel(config().diagLevel()), _useSubsystem0(config().useSubsystem0()) {
...
void CrvDigisFromArtdaqFragments::produce(art::Event& event) {
  ...
  if (_diagLevel > 1) {
    std::cout << std::dec << "Run/Subrun/Event: " << event.run() << "/" << event.subRun() << "/"
  ...
  for (const auto& fragmentHandle : fragmentHandles) {
    ...
    for (auto artFragment : *fragmentHandle) {
      if (artFragment.type() != mu2e::FragmentType::DTCEVT)
        continue;
      ...
      // collect sub events
      auto dtcSubEvents =
          dtcEventFragment.getSubsystemData(DTCLib::DTC_Subsystem::DTC_Subsystem_CRV);
      if (_useSubsystem0) {
        auto dtcSubEventsTmp = dtcEventFragment.getSubsystemData(
            DTCLib::DTC_Subsystem::DTC_Subsystem_Tracker); // currently wrongly encoded in the DTC
                                                           // Subevent header
        dtcSubEvents.insert(
            dtcSubEvents.end(), dtcSubEventsTmp.begin(),
            dtcSubEventsTmp.end()); // temporarily add fragments with wrongly encoded headers
      }
      ...
      for (size_t iSubEvent = 0; iSubEvent < dtcSubEvents.size(); ++iSubEvent)
        actualSize += dtcSubEvents.at(iSubEvent).GetSubEventByteCount();
      if (_diagLevel > 1) {
        std::cout << "Fragment index: " << iFragment
                  << "      expected event size: " << expectedSize
                  << ", actual event size: " << actualSize << std::endl;
      }
      ...
      for (size_t iSubEvent = 0; auto& dtcSubEvent : dtcSubEvents) {
        ...
        for (size_t iDataBlock = 0; iDataBlock < decoder.block_count(); ++iDataBlock) {
          ...
          if (block == nullptr) {
            ...
            crvDaqErrors->emplace_back(mu2e::CrvDAQerrorCode::unableToGetDataBlock, iFragment,
                                       iSubEvent, iDataBlock, 0);
            continue;
          }
          ...
          if (!header->isValid()) {
            ...
            std::cerr << "sub system ID: " << (uint16_t)header->GetSubsystemID()
                      << " packet count: " << header->GetPacketCount() << std::endl;
            crvDaqErrors->emplace_back(mu2e::CrvDAQerrorCode::invalidPacket, iFragment, iSubEvent,
                                       iDataBlock, header->GetPacketCount());
            continue;
          }
          ...
          if (header->GetSubsystemID() != DTCLib::DTC_Subsystem::DTC_Subsystem_CRV) {
            if (_diagLevel > 0) {
              ...
              std::cout << "sub system ID: " << (uint16_t)header->GetSubsystemID()
                        << " packet count: " << header->GetPacketCount() << std::endl;
            }
            crvDaqErrors->emplace_back(mu2e::CrvDAQerrorCode::wrongSubsystemID, iFragment,
                                       iSubEvent, iDataBlock, header->GetPacketCount());
            continue;
          }
          ...
          if (header->GetPacketCount() > 0) {
            ...
            if (crvRocHeader == nullptr) {
              ...
              crvDaqErrors->emplace_back(mu2e::CrvDAQerrorCode::errorUnpackingStatusPacket,
                                         iFragment, iSubEvent, iDataBlock,
            ...
            if (!decoder.GetCRVHits(iDataBlock, crvHits)) {
              ...
              crvDaqErrors->emplace_back(mu2e::CrvDAQerrorCode::errorUnpackingCrvHits, iFragment,
                                         iSubEvent, iDataBlock, header->GetPacketCount());
              break;
            }
            for (auto const& crvHit : crvHits) {
              ...
              for (size_t i = 0; i < waveform.size(); ++i) {
                if ((adc[i] & 0x800) == 0x800)
                  adc[i] = (int16_t)(adc[i] | 0xF000);
              } // to handle negative numbers stored in 12bit ADC samples
              crvDigis->emplace_back(adc, crvHitInfo.HitTime, false,
                                     mu2e::CRSScintillatorBarIndex(crvBarIndex), SiPMNumber);
              crvDigisNZS->emplace_back(adc, crvHitInfo.HitTime, true,
                                        mu2e::CRSScintillatorBarIndex(crvBarIndex),
                                        SiPMNumber); // temporary solution until we get the FEB-II
            ...
            if (_diagLevel > 2) {
              ...
              std::cout << "MicroBunchStatus: " << std::hex
                        << (uint16_t)crvRocHeader->MicroBunchStatus << std::dec << std::endl;
            ...
            if (_diagLevel > 3) {
              for (auto const& crvHit : crvHits) {
                ...
                std::cout << "ROCID (increased by 1 to match the Online/Offline-Channel Map) "
                          << rocID << "   rocPort " << rocPort << "   febChannel " << febChannel
