...
public:
  struct Config {
    ...
    fhicl::Atom<double> width_guess{
        fhicl::Name("width_guess"),
        fhicl::Comment("Estimate of pulse width for fit (default = 150e-9)")};
    fhicl::Atom<double> rise_time_guess{
        fhicl::Name("rise_time_guess"),
        fhicl::Comment("Estimate of pulse rise time for fit (default = 6e-9)")};
    fhicl::Atom<double> threshold{fhicl::Name("threshold"),
                                  fhicl::Comment("Threshold to define the peak (default = 20000)")};
    fhicl::Atom<double> samp_freq{fhicl::Name("samp_freq"),
                                  fhicl::Comment("Sampling frequency of ADC (default = 300e6)")};
    fhicl::Atom<int> pulse_region{
        fhicl::Name("pulse_region"),
        fhicl::Comment("Number of ADC values to fit around pulse (default 51)")};
    fhicl::Atom<int> pulses_per_event{
        fhicl::Name("pulses_per_event"),
        fhicl::Comment("Number of pulses per event to find Delta t (default = 2)")};
  };
...
// Fitting function
double STMWaveformDigisFromFragments::fit_rising_edge(std::vector<double> x, std::vector<double> y,
                                                      double pulse_time) {
...
std::vector<double> STMWaveformDigisFromFragments::linspace(double start_in, double end_in,
                                                            int num_in) {
...
STMWaveformDigisFromFragments::STMWaveformDigisFromFragments(
    const art::EDProducer::Table<Config>& config) :
    art::EDProducer{config},
    _stmFragmentsTag(config().stmTag()), _width_guess(config().width_guess()),
    _rise_time_guess(config().rise_time_guess()), _threshold(config().threshold()),
    _samp_freq(config().samp_freq()), _pulse_region(config().pulse_region()),
...
void STMWaveformDigisFromFragments::produce(Event& event) {
  eventCounter++;
  std::unique_ptr<mu2e::STMWaveformDigiCollection> stm_waveform_digis(
      new mu2e::STMWaveformDigiCollection);
  ...
  for (const auto& frag : *STMFragments) {
    ...
    uint64_t DTC = static_cast<uint64_t>(DTC3) << 40 | static_cast<uint64_t>(DTC2) << 24 |
                   static_cast<uint64_t>(DTC1) << 8 | static_cast<uint64_t>(DTC0);
    // std::cout << "DTCs: " << std::hex << DTC0 << " " << DTC1 << " " << DTC2 << " " << DTC3 << "
    // --> " << DTC << std::dec << std::endl; std::cout << "DTC time = " << DTC << " = " <<
    // (DTC/40e6) << "\n";

    // Get 75MHz ADC clock time
    int16_t adc[4] = {*(stm_frag.GetTHdr() + 4), *(stm_frag.GetTHdr() + 5),
                      *(stm_frag.GetTHdr() + 6), *(stm_frag.GetTHdr() + 7)};
    uint64_t ADC;
    ...
    for (int16_t i_adc_sample = 0; i_adc_sample < event_len; ++i_adc_sample) {
    ...
    std::cout << "pulse 0 = " << peak_fitTime1 << "  |  pulse 1 = " << peak_fitTime2
              << "  |  pulse sep = " << peak_sep << "\n";
    ...
    mu2e::STMWaveformDigi stm_waveform(detID, EWT, DTC, ADC, trig_time_offset, peak_fitTime1,
                                       peak_fitTime2, peak_sep, adcs);
