...
public:
  struct Config {
    ...
    fhicl::Atom<float>         digiSampling {fhicl::Name("digiSampling") , fhicl::Comment("calorimeter sampling period in ns")};
    fhicl::Atom<float>         deltaTPulses {fhicl::Name("deltaTPulses") , fhicl::Comment("time-gate between two signal from different SiPMs coupled with the same crystal")};
    fhicl::Atom<float>         pulseRatioMax{fhicl::Name("pulseRatioMax"), fhicl::Comment("Max of the ratio between two SiPM pulses coupled to the same crystal within the time-gate")};
    fhicl::Atom<float>         pulseRatioMin{fhicl::Name("pulseRatioMin"), fhicl::Comment("Min of the ratio between two SiPM pulses coupled to the same crystal within the time-gate")};
  };
...
void art::CaloHitsFromFragments::produce(Event& event) {
  for(size_t i=0; i<pulseMap_.size(); ++i){
    pulseMap_[i].clear();
  }
  ...
  if (!calFragments.isValid()) {
    std::cout << "[CaloHitsFromFragments::produce] found no Calorimeter fragments!"
	      << std::endl;
  ...
  if (diagLevel_ > 0) {
    std::cout << "mu2e::CaloHitsFromFragments::produce exiting eventNumber="
...
void art::CaloHitsFromFragments::analyze_calorimeter_(const artdaq::Fragment& f,
						      std::unique_ptr<mu2e::CaloHitCollection> const& calo_hits,
						      std::unique_ptr<mu2e::CaloHitCollection> const& caphri_hits) {
  ...
  for (size_t curBlockIdx = 0; curBlockIdx < cc.block_count(); curBlockIdx++) {
    ...
    if (diagLevel_ > 0) {
      std::cout << "[CaloHitsFromFragments] NEW CALDATA: NumberOfHits "
	...
	caloDAQUtil_.printAllHitInfo(crystalID, sipmID, hdr, hits[hitIdx].first, hits[hitIdx].second);
      } // End debug output
