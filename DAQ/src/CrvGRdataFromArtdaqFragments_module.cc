...
public:
  struct Config {
    ...
    fhicl::Atom<std::string> csvFileName{fhicl::Name("csvFileName"),
                                         fhicl::Comment("csv file name")};
...
CrvGRdataFromArtdaqFragments::CrvGRdataFromArtdaqFragments(
    ...
    _diagLevel(config().diagLevel()), _csvFileName(config().csvFileName()),
    _writeCsvFile(config().writeCsvFile()), _useSubsystem0(config().useSubsystem0()) {
  if (_writeCsvFile) {
        ...
        for (size_t iDataBlock = 0; iDataBlock < decoder.block_count(); ++iDataBlock) {
          ...
          if (header->GetPacketCount() > 0) {
            ...
            std::unique_ptr<mu2e::CRVDataDecoder::CRVROCStatusPacket> crvRocHeader =
                decoder.GetCRVROCStatusPacket(iDataBlock);
            ...
            if (crvRocHeader != nullptr && _diagLevel > 0) {
              ...
              std::cout << "ROCID (ROC Status): " << (uint16_t)crvRocHeader->ControllerID
                        << std::endl;
              ...
              std::cout << "EventWindowTag (ROC Status): "
                        << crvRocHeader->GetEventWindowTagGlobalRun() << std::endl;
              std::cout << std::endl;
            }
            ...
            if (decoder.GetCRVGlobalRunPayload(iDataBlock, globalRunPayload) && _diagLevel > 0) {
              for (size_t iWord = 0; iWord < globalRunPayload.size(); ++iWord) {
                if (iWord % 8 == 0)
                  std::cout << "**** PACKET " << iWord / 8 + 2 << " **** Payload **************"
                            << std::endl;
            ...
            if (_writeCsvFile) {
              _csvFile << event.event() << "," << iSubEvent << "," << iDataBlock
                       << ","; // event number, sub event, data block
