...
LumiInfoAna::LumiInfoAna(const art::EDAnalyzer::Table<Config>& config) :
    art::EDAnalyzer{config}, _diagLevel(config().diagLevel()), _nevents(0), _nsubrun(0) {}
...
void LumiInfoAna::beginJob() {
  ...
  _hNCaloHits =
      infoDir.make<TH1F>("hNCaloHits", "N(Calorimeter hits);N(hits);Events", 100, 0., 100.);
  _hCaloEnergy =
      infoDir.make<TH1F>("hCaloEnergy", "Calorimeter energy;Energy (MeV);Events", 100, 0., 1000.);
  _hNCaphriHits =
      infoDir.make<TH1F>("hNCaphriHits", "N(CAPHRI hits);N(hits);Events", 100, 0., 100.);
  _hNTrackerHits =
      infoDir.make<TH1F>("hNTrackerHits", "N(tracker hits);N(hits);Events", 100, 0., 10000.);
  _hNTimeClusters = infoDir.make<TH1F>("hNTimeClusters", "N(time clusters);N(time clusters);Events",
                                       100, 0., 100.);
  _hSubRuns = infoDir.make<TH1F>("hSubRuns", "Sub-Runs;Sub-Run;Events", 100, 0., 100.);
}

void LumiInfoAna::endJob() {
  std::cout << "LumiInfoAna::" << __func__ << ": Read information for an accumulated " << _nevents
            << " events\n";
}
    ...
    if (!handle.isValid() || handle->empty()) {
      if (_diagLevel > 0) {
        std::cout << "LumiInfoAna::" << __func__ << ": Invalid or empty calo info handle!"
                  ...
                  << " and size = " << ((handle.isValid()) ? ((int)handle->size()) : -1)
                  << std::endl;
      }
      continue;
    }
...
//--------------------------------------------------------------------------------
void LumiInfoAna::fillTrackerHits(
    std::vector<art::Handle<mu2e::IntensityInfosTrackerHits>>& handles) {
  if (_diagLevel > 0) {
    std::cout << "LumiInfoAna::" << __func__ << ": Processing " << handles.size() << " handles"
              << std::endl;
  }
...
//--------------------------------------------------------------------------------
void LumiInfoAna::fillTimeClusters(
    std::vector<art::Handle<mu2e::IntensityInfosTimeCluster>>& handles) {
  if (_diagLevel > 0) {
    std::cout << "LumiInfoAna::" << __func__ << ": Processing " << handles.size() << " handles"
              << std::endl;
  }
...
void LumiInfoAna::endSubRun(const art::SubRun& sr) {
  ...
  if (_diagLevel > 0) {
    ...
    if (_diagLevel > 1) {
      ...
      for (auto entry : _counter_by_object)
        std::cout << "  " << entry.first.c_str() << " : " << entry.second << std::endl;
    }
  }
  _nsubrun = 0;
}
...
void LumiInfoAna::analyze(const art::Event& event) {
  ...
  if (_diagLevel > 0) {
    std::cout << "LumiInfoAna::" << __func__ << ": Subrun " << runNumber << ":" << subrunNumber
              << ":" << eventNumber << ": Retrieved " << caloHandles.size() << " calo handles"
              << " " << trkHandles.size() << " tracker hits handles"
              << " " << tcHandles.size() << " time cluster handles"
              << " " << headerHandles.size() << " header handles" << std::endl;
  }
