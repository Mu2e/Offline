...
public:
  struct Config {
    ...
    fhicl::Atom<int> simMode{fhicl::Name("simMode"),
                             fhicl::Comment("Simulation mode: 0 = all zeros; 1 = non-zero values"),
                             0};
  };
...
DummyLumiInfoProducer::DummyLumiInfoProducer(const art::EDProducer::Table<Config>& config) :
    art::EDProducer{config}, _diagLevel(config().diagLevel()), _simMode(config().simMode()) {
...
void DummyLumiInfoProducer::produce(art::Event& event) {
  ...
  if (_diagLevel > 1)
    std::cout << "DummyLumiInfoProducer::" << __func__ << ": Event " << runNumber << ":"
  ...
  auto timeClusterInfo =
      std::unique_ptr<mu2e::IntensityInfoTimeCluster>(new mu2e::IntensityInfoTimeCluster);
  auto trackerInfo =
      std::unique_ptr<mu2e::IntensityInfoTrackerHits>(new mu2e::IntensityInfoTrackerHits);

  // Assign non-zero entries for harder compression
  if (_simMode == 1) {
    const static int prime_1(17), prime_2(251), prime_3(503),
        prime_4(1523); // for distributing values somewhat evenly
