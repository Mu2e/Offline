...
void art::CaloDigiFromDTCEvents::produce(Event& event) {
  ...
  if (diagLevel_ > 1) {
    std::cout << std::dec << "[CaloDigiFromDTCEvents::produce] Run " << event.run() << ", subrun " << event.subRun()
    ...
    if (data_type_ == 0){ //Standard calo data
      ...
      for (unsigned int hitIdx = 0; hitIdx < calHitDataVec->size(); hitIdx++) {
        ...
        if (diagLevel_ > 2) {
          ...
          std::cout << "[CaloDigiFromDTCEvents] \tChNumber   "
          ...
          std::cout << "[CaloDigiFromDTCEvents] \tErrorFlags " << (int)thisHitPacket.ErrorFlags
            << std::endl;
          std::cout << "[CaloDigiFromDTCEvents] \tTime              "
            << (int)thisHitPacket.Time << std::endl;
          std::cout << "[CaloDigiFromDTCEvents] \tNSamples   "
      ...
      for (unsigned int hitIdx = 0; hitIdx < calHitTestDataVec->size(); hitIdx++) {
        ...
        if (errorCode){
          ...
          total_hits_bad++;
          if (diagLevel_ > 1){
            std::cout << "[CaloDigiFromDTCEvents] BAD calo hit! DTC: " << dtcID << ", ROC: " << iROC << ", hit number: " << hitIdx << " [failure code: " << errorCode << "]" << std::endl;
            caloDAQUtil_.printCaloPulse(thisHitPacket);
            std::cout << "[CaloDigiFromDTCEvents] \twaveform size \t" << thisHitWaveform.size() << std::endl;
          }
          continue;
        }
