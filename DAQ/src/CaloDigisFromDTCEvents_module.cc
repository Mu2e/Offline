// ======================================================================
//
// CaloDigiFromDTCEvents_plugin:  Add cal data products to the event
//
// ======================================================================
...
namespace art {
class CaloDigiFromDTCEvents;
}

// ======================================================================

class art::CaloDigiFromDTCEvents : public EDProducer {
...
public:
  struct Config {
    ...
    fhicl::Atom<bool> useOfflineID{
        fhicl::Name("useOfflineID"),
        fhicl::Comment("Use calo disk mapping for SiPM IDs (Default: true)"), true};
    fhicl::Atom<bool> useDTCROCID{
        fhicl::Name("useDTCROCID"),
        fhicl::Comment("Use DTC and ROC numbers instead of boardID for SiPM IDs (Default: false)"),
        false};
  };

  // --- C'tor/d'tor:
  explicit CaloDigiFromDTCEvents(const art::EDProducer::Table<Config>& config);
  virtual ~CaloDigiFromDTCEvents() {}
...
// ======================================================================
art::CaloDigiFromDTCEvents::CaloDigiFromDTCEvents(const art::EDProducer::Table<Config>& config) :
    art::EDProducer{config}, data_type_(config().data_type()), diagLevel_(config().diagLevel()),
    caloDAQUtil_("CaloDigiFromDTCEvents"), useOfflineID_(config().useOfflineID()),
...
// ----------------------------------------------------------------------
void art::CaloDigiFromDTCEvents::produce(Event& event) {
  ...
  if (diagLevel_ > 1) {
    std::cout << std::dec << "[CaloDigiFromDTCEvents::produce] Run " << event.run() << ", subrun "
...
void art::CaloDigiFromDTCEvents::endJob() {

  if (diagLevel_ > 0) {
    std::cout << "\n ----- [CaloDigiFromDTCEvents] Decoding errors summary ----- " << std::endl;
...
DEFINE_ART_MODULE(art::CaloDigiFromDTCEvents)
