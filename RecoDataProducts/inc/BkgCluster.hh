...
namespace mu2e
{
   struct BkgCluster
   {
       ...
       BkgCluster(TwoDPoint point, float time) : _point(point), _time(time) {_hits.reserve(16);}

       float                        getKerasQ() const {return _kerasQ; }
       auto const&        flag() const {return _flag; }
       auto const&                pos()  const {return _pos;  }
       auto const                 cpos()  const {return _point.pos3();  }
        auto const&                point()  const {return _point;  }
