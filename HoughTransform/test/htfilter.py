...
process.TFileService = mu2e.Service("TFileService",
                       fileName = mu2e.string("houghtest_0501.root"),
...
process.outfile = mu2e.OutputModule(
    "PoolOutputModule",
    fileName = cms.untracked.string('file:hough_0501.root'),
)
...
process.output = mu2e.EndPath(  process.generate*process.g4run*process.simpleen*process.checkhits*process.plothits*process.outfile );
