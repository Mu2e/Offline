...
*CrvList2DXZ)
  {
    ...
    if(sposi.x()!=0){
     ...
     CrvList2DXY->AddElement(line_twoDstrawXY);
      ...
      line_twoDstrawXZ->SetPoint(0,pointmmTocm(sposi.x()),pointmmTocm(sposi.y())+1000,pointmmTocm(sposi.z())+18000);
      line_twoDstrawXZ->SetNextPoint(pointmmTocm(sposf.x()),pointmmTocm(sposf.y())+1000,pointmmTocm(sposf.z())+18000);
