...
namespace mu2e{
  ...
  Bool_t TEveMu2eMainWindow::ProcessMessage(Long_t msg, Long_t param1, Long_t param2){
    ...
    switch (GET_SUBMSG(msg)){
      ...
      if (param1 == 1702){   
        ...
        if (fhitmin < fhitmax) {*hitenergy = pass_data->AddTrkHits(_firstLoop, _data.chcol, data.track_tuple, tracker2Dproj, false, fhitmin, fhitmax,ftimemin, ftimemax, _accumulate, TfXYMgr, TfRZMgr, proj2, proj3);}
	      if (fhitmin > fhitmax){
 ...
 if (param1 == 1703){
        ...
        if (ftimemin < ftimemax) {
          ...
          *hitenergy = pass_data->AddTrkHits(_firstLoop, _data.chcol, data.track_tuple, tracker2Dproj, false, fhitmin, fhitmax, ftimemin, ftimemax, _accumulate, TfXYMgr, TfRZMgr, proj2, proj3);
