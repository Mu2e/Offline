...
Double_t gausExpTails(Double_t *x, Double_t *par){
	  Double_t res=0.0;
	  //Double_t Aexp=pow(10.,par[6]);
	  //res = par[0]*TMath::Gaus(x[0],par[1], par[2], kTRUE);
	  //res += (1.0-par[0]-Aexp)*TMath::Gaus(x[0],par[3], par[4], kTRUE);
	  //res += Aexp*0.5/par[7]*TMath::Exp(-1.0*TMath::Abs((x[0]-par[1]))/par[7]);
	  //res *= par[5];

	  Double_t invTau1 = 1.0/par[4];
	  Double_t invTau2 = 1.0/par[5];
	  if (par[3]<0.0) par[3]=0.0;
	  if (par[3]>1.0) par[3]=1.0;
	  Double_t fracExp = (1.0-par[3])/(1.0+par[5]);
	  fracExp*=invTau1;

	  Double_t xEx = x[0]-par[1];
	  if (xEx>0.0) res+=fracExp*TMath::Exp(-xEx*invTau1);
	  else res+=fracExp*TMath::Exp(xEx*invTau2);
	  res+=par[3]*TMath::Gaus(x[0],par[1], par[2], kTRUE);
	  res*=par[0];
...
void MomRes(TTree* trk) {
  ...
  TCanvas* mpullcan = new TCanvas("mpullcan","Momentum Pull",1200,800);
  gStyle->SetOptStat(111111);
  gStyle->SetOptFit(111111);
// should have pitch angle and generated hit cuts here, FIXME!!!
//  TCut mcsel("fitinfo.momin>100&&startpar.paramVec.m[4]<1.0&&startpar.paramVec.m[4]>0.5774&&nmc>=15");
  TCut mcsel("fitinfo.momin>100");//&&startpar.paramVec.m[4]<1.0&&startpar.paramVec.m[4]>0.5774");//&&abs(tpeak-mcmidt0-28)<30");
  TCut tsel = mcsel +TCut("fitinfo.fit==1");
// selection cuts
  TCut cuts[4];

  TH1F* effnorm = new TH1F("effnorm","effnorm",100,0,150);
  trk->Project("effnorm","fitinfo.momin",mcsel);
  
  cout<<"N events that enter the chamber with more than 100 MeV/c "<<effnorm->GetEntries()<<endl;
  ...
  TF1* gauExpTls = new TF1("gauExpTls",gausExpTails,-1.,1.,6);
  gauExpTls->SetParName(0,"Norm");
  gauExpTls->SetParName(1,"Mean");
  gauExpTls->SetParName(2,"sig");
  gauExpTls->SetParName(3,"frac_{core}");
  gauExpTls->SetParName(4,"TauH");
  gauExpTls->SetParName(5,"TauL");
...
// cuts for different tightness of selection
  ...
  momcuts[0] = "";
  momcuts[1] = "fitinfo.fitmomerr<0.4";
  momcuts[2] = "fitinfo.fitmomerr<0.35";
  momcuts[3] = "fitinfo.fitmomerr<0.3";
  fitcuts[0] = "";
  fitcuts[1] = "fitprob>1e-4";
  fitcuts[2] = "fitprob>1e-3";
  fitcuts[3] = "fitprob>1e-2";
  TCut fitcutsEffct[4];
  fitcutsEffct[0] = "";
  fitcutsEffct[1] = "TMath::Prob(fitinfo.chi2,(fitinfo.nhits-5))>1e-4";
  fitcutsEffct[2] = "TMath::Prob(fitinfo.chi2,(fitinfo.nhits-5))>1e-3";
  fitcutsEffct[3] = "TMath::Prob(fitinfo.chi2,(fitinfo.nhits-5))>1e-2";
  ...
  for(unsigned ires=0;ires<4;ires++){
    char mname[50];
    snprintf(mname,50,"momres%i",ires);
    momres[ires] = new TH1F(mname,"momentum resolution at start of tracker;MeV/c",151,-2.5,2.5);
    trk->Project(mname,"fitinfo.fitmom-fitinfo.momin",ncuts[ires]/*+t0cuts[ires]+momcuts[ires]*/+fitcutsEffct[ires]+tsel);
    double integral = momres[ires]->GetEntries()*momres[ires]->GetBinWidth(1);
    /*sgau->SetParameters(integral,0.0,momres[ires]->GetRMS(),momres[ires]->GetRMS(),0.01,2*momres[ires]->GetRMS(),2*momres[ires]->GetRMS());
    sgau->SetParLimits(5,1.0*momres[ires]->GetRMS(),1.0);
    sgau->SetParLimits(6,1.0*momres[ires]->GetRMS(),1.0);
    sgau->SetParLimits(4,0.0,0.8);
    mcan->cd(ires+1)->SetLogy(kTRUE);
    momres[ires]->Fit("sgau","L");*/
    gauExpTls->SetParameters(integral,0.0,momres[ires]->GetRMS()*0.5,0.8,0.4,0.6);
    gauExpTls->SetParLimits(1,-0.2,0.4);
    gauExpTls->SetParLimits(2,0.0,momres[ires]->GetRMS());
    gauExpTls->SetParLimits(3,0.0,1.0);
    gauExpTls->SetParLimits(4,0.0,2.0*momres[ires]->GetRMS());
    gauExpTls->SetParLimits(5,0.0,3.0*momres[ires]->GetRMS());
    mcan->cd(ires+1)->SetLogy(kTRUE);
    momres[ires]->Fit("gauExpTls","L");
    ...
    momres[ires] = new TH1F(mname,"momentum pull at start of tracker",250,-5,5);
    trk->Project(mname,"(fitinfo.fitmom-fitinfo.momin)/fitinfo.fitmomerr",ncuts[ires]/*+t0cuts[ires]+momcuts[ires]*/+fitcutsEffct[ires]+tsel);
    mpullcan->cd(ires+1);
    momres[ires]->Fit("gaus");
