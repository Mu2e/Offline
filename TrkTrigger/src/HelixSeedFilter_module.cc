...
namespace mu2e 
{
  class HelixSeedFilter : public art::EDFilter 
  {
    public:
      explicit HelixSeedFilter(fhicl::ParameterSet const& pset);
  ...
  HelixSeedFilter::HelixSeedFilter(fhicl::ParameterSet const& pset) :
    ...
    _minmom(pset.get<double>("MinMomentum",270.0)),
    _maxmom(pset.get<double>("MaxMomentum",390.0)) ,
    _goodh(pset.get<vector<string> >("HelixFitFlag",vector<string>{"HelixOK"})),
    _nevt(0), _npass(0)
  {
  }

  bool HelixSeedFilter::filter(art::Event& evt){
    ++_nevt;
...
// should create a trigger product output FIXME
  ...
  void HelixSeedFilter::endJob() {
    if(_nevt > 0){
      cout << "HelixSeedFilter passed " <<  _npass << " events out of " << _nevt << " for a ratio of " << float(_npass)/float(_nevt) << endl;
    }
