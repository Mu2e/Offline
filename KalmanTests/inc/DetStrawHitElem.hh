...
// define the type for this class first  
  ...
  public:
    DetStrawHitType(elemType etype) {
      if(etype == wall)
        _mat = _matdbinfo->findDetMaterial("straw-wall");
      else if(etype == gas)
        _mat = _matdbinfo->findDetMaterial("straw-gas"); }
...
// construct from a TrkStrawHit
    DetStrawHitElem(elemType etype) : DetElem(new DetStrawHitType(etype),"DetStrawHitElem",_ielem++) {}
