...
// simple structs
  ...
  class KalFit : public art::EDProducer
  {
  public:
...
// ugly place to put this
  ...
  KalFit::KalFit(fhicl::ParameterSet const& pset) : _wallelem(wall),_gaselem(gas),
    ...
    _printfreq(pset.get<unsigned>("printfrequency",1)),
...
// field map service, FIXME!!!!
  ...
  void KalFit::produce(art::Event& event ) 
  {
// event printout
...
// find the input data objects needed to make tracks
  bool KalFit::findData(art::Event& evt){
    _strawhits = 0;
...
// find the MC truth objects in the event and set the local cache
  bool KalFit::findMC(art::Event& evt) {
