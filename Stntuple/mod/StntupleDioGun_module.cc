...
namespace mu2e {
  ...
  //================================================================
  class StntupleDioGun : public art::EDProducer {
  ...
  public:
    explicit StntupleDioGun(const fhicl::ParameterSet& pset);

    virtual void produce(art::Event& event);
  };

  //================================================================
  StntupleDioGun::StntupleDioGun(const fhicl::ParameterSet& pset)
    ...
    , stops_(eng_, pset.get<fhicl::ParameterSet>("muonStops"), sizeof(InputStop)/sizeof(float))
  {
    ...
    if(verbosityLevel_ > 0) {
      std::cout<<"StntupleDioGun: using = "
               <<stops_.numRecords()
               <<" stopped particles"
               <<std::endl;

      std::cout<<"StntupleDioGun: producing particle "
               <<pdgId_
               <<", mass = "<<mass_
               <<std::endl;

      std::cout <<"StntupleDioGun: spectrum shape = "
    ...
    if(verbosityLevel_ > 1){
      std::cout <<"StntupleDioGun: spectrum: " << std::endl;
      spectrum_.print();
    }
  ...
  //================================================================
  StntupleDioGun::SpectrumVar
  StntupleDioGun::parseSpectrumVar(const std::string& name) {
    if(name == "totalEnergy")  return TOTAL_ENERGY;
    if(name == "kineticEnergy")  return KINETIC_ENERY;
    throw cet::exception("BADCONFIG")<<"StntupleDioGun: unknown spectrum variable "<<name<<"\n";
  }

  //================================================================
  BinnedSpectrum
  StntupleDioGun::parseSpectrumShape(const fhicl::ParameterSet& psphys,
                                                 PDGCode::type pdgId,
                                                 double *elow,
                                                 double *ehi)
  {
    ...
    if (spectrumShape == "Czarnecki") {
      ...
      *ehi = res.getAbscissa(res.getNbins()-1);
      ...
      throw cet::exception("BADCONFIG")
        << "StntupleDioGun: unknown spectrum shape "<<spectrumShape<<"\n";
    }
  ...
  //================================================================
  void StntupleDioGun::produce(art::Event& event) {
  ...
  //================================================================
  double StntupleDioGun::generateEnergy() {
...
DEFINE_ART_MODULE(mu2e::StntupleDioGun);
