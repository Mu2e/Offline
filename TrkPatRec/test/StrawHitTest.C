...
//    ephot->Draw("same");
    ...
    TBox* eselbox = new TBox(0.0,edelta->GetMinimum(),3.0,edelta->GetMaximum());
...
//    rmax_l->Draw();
    ...
    dtime->Scale(scale);
    sorigin->Add(dtime);
    hits->Project("mtime","time",ootmuon+hitsel);
    mtime->Scale(scale);
    sorigin->Add(mtime);
    ...
    gtime->Scale(scale);
    sorigin->Add(gtime);
    hits->Project("stptime","time",stpproton+hitsel);
    stptime->Scale(scale);
    sorigin->Add(stptime);
    hits->Project("pptime","time",pproton+hitsel);
    pptime->Scale(scale);
    sorigin->Add(pptime);
