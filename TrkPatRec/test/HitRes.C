...
//  TH1F* ndt = new TH1F("ndt","Non-stereo #Delta t;t_{sh}-t_{MC}",100,-100,100);
  ...
  shdiag->Project("sdp","atan2(stpos.y,stpos.x)-atan2(mcshpos.y,mcshpos.x)",stereo);
  shdiag->Project("ndp","atan2(stpos.y,stpos.x)-atan2(mcshpos.y,mcshpos.x)",nstereo);
  shdiag->Project("sdrp","atan2(stpos.y,stpos.x)*sqrt(stpos.y^2+stpos.x^2)-atan2(mcshpos.y,mcshpos.x)*sqrt(mcshpos.y^2+mcshpos.x^2)",stereo);
  shdiag->Project("ndrp","atan2(stpos.y,stpos.x)*sqrt(stpos.y^2+stpos.x^2)-atan2(mcshpos.y,mcshpos.x)*sqrt(mcshpos.y^2+mcshpos.x^2)",nstereo);
  shdiag->Project("sdr","sqrt(stpos.y^2+stpos.x^2)-sqrt(mcshpos.y^2+mcshpos.x^2)",stereo);
  shdiag->Project("ndr","sqrt(stpos.y^2+stpos.x^2)-sqrt(mcshpos.y^2+mcshpos.x^2)",nstereo);
//  shdiag->Project("sdt","time-mctime",stereo);
//  shdiag->Project("ndt","time-mctime",nstereo);
  shdiag->Project("sdx","stpos.x-mcshpos.x",stereo);
  shdiag->Project("ndx","stpos.x-mcshpos.x",nstereo);
