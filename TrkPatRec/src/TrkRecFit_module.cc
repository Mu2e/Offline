...
//  Info depending on stereo hits
  ...
  void TrkRecFit::fillStrawHitInfo(size_t ish, StrawHitInfo& shinfo) const {
    const Tracker& tracker = getTrackerOrThrow();
...
// info depending on stereo hits
  ...
  void TrkRecFit::fillPeakDiag(size_t ip,TrkTimePeak const& tp) {
    _tpeventid = _eventid;
    _peakid = ip;
    _pmax = tp._peakmax;
  ...
  void TrkRecFit::initializeReaders() {
