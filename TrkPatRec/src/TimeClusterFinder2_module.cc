...
//
// $Id: TimeClusterFinder2_module.cc,v 1.3 2014/08/25 12:08:29 tassiell Exp $
// $Author: tassiell $
...
namespace mu2e {
  ...
  class TimeClusterFinder2 : public art::EDProducer {
    public:
      enum ClusterAlgorithm {peak=0,scan};
      explicit TimeClusterFinder2(fhicl::ParameterSet const& pset);
      virtual ~TimeClusterFinder2();
    ...
    TimeClusterFinder2::~TimeClusterFinder2() {
    }

    TimeClusterFinder2::TimeClusterFinder2(fhicl::ParameterSet const& pset) :
    ...
    void TimeClusterFinder2::beginJob()
    {
       _peakMVA.initMVA();
       if (_debug > 0)
       {
         std::cout << "TimeClusterFinder2 MVA : " << std::endl; 
         _peakMVA.showMVA();
       }
    }
    ...
    //--------------------------------------------------------------------------------------------------------------
    void TimeClusterFinder2::produce(art::Event & event )
    {     
       if (_debug > 0 && (event.id().event()%_printfreq)==0) std::cout<<"TimeClusterFinder2: event="<<event.id().event()<<std::endl;
    ...
    //--------------------------------------------------------------------------------------------------------------
    void TimeClusterFinder2::findClusters(TimeClusterCollection& tclusts, StrawHitFlagCollection& flags, 
                                          const StrawHitCollection& shcol, const StrawHitPositionCollection& shpcol, 
                                          const StrawHitFlagCollection& shfcol, 
                                          const art::Handle<CaloClusterCollection> CaloClusterHandle, 
                                          const CaloClusterCollection* caloClusters) 
    ...
    //--------------------------------------------------------------------------------------------------------------
    void TimeClusterFinder2::fillTimeSpectrum(const StrawHitCollection& shcol, const StrawHitPositionCollection& shpcol,
                                              const std::vector<int>& goodHitFlag, const CaloClusterCollection* caloClusters)
    {
        _timespec.Reset();
    ...
    //--------------------------------------------------------------------------------------------------------------
    void TimeClusterFinder2::findPeaks(std::vector<double>& tctimes)
    {
       tctimes.clear();
    ...
    //--------------------------------------------------------------------------------------------------------------
    void TimeClusterFinder2::scanPeaks(std::vector<double>& tctimes)
    {
    ...
    //--------------------------------------------------------------------------------------------------------------
    void TimeClusterFinder2::initCluster(TimeCluster& tclust, const StrawHitCollection& shcol, 
    ...
    //--------------------------------------------------------------------------------------------------------------
    void TimeClusterFinder2::addCaloCluster(TimeCluster& tc, const art::Handle<CaloClusterCollection> CaloClusterHandle, 
                                            const CaloClusterCollection* caloClusters) 
    {
    ...
    //--------------------------------------------------------------------------------------------------------------
    bool TimeClusterFinder2::goodHit(const StrawHitFlag& flag) const 
    {
       return flag.hasAllProperties(_hsel) && !flag.hasAnyProperty(_hbkg);
    }

    //--------------------------------------------------------------------------------------------------------------
    double TimeClusterFinder2::clusterWeight(const TimeCluster& tc) const
    {
...
using mu2e::TimeClusterFinder2;
DEFINE_ART_MODULE(TimeClusterFinder2);
...
/*
       while (tclust._strawHitIdxs.size() >= _minnhits) {
	     ...
	     _pmva._rho = rho;
             ...
             //precut on this variable, very good discriminant 
             if (std::abs(dphi) > _maxdPhi) {batchremove.push_back(ips);continue;}
          ...
          if (worstmva < _minpeakmva) batchremove.push_back(iworst);
          if (batchremove.empty()) break;
          
          for (auto irm=batchremove.rbegin();irm!=batchremove.rend();++irm)
          {
	     std::swap(tclust._strawHitIdxs[*irm],tclust._strawHitIdxs.back());
	     tclust._strawHitIdxs.pop_back();      
          }
      ...
      while(tclust._strawHitIdxs.size() >= _minnhits)
      {
	    ...
	    if (mvaout < worstmva)
	       ...
	       iworst = ips;
            ...
            if (mvaout < _minpeakmva-0.2) batchremove.push_back(ips); //change 0.2 into parameter
         }
         ...
         {
            firstPass=false;
            std::sort(batchremove.begin(),batchremove.end(),std::greater<size_t>());
            for (auto& i : batchremove)
            {
	      std::swap(tclust._strawHitIdxs[i],tclust._strawHitIdxs.back());
	      tclust._strawHitIdxs.pop_back();      
            }
         } else {      
...
//another preilter
          ...
          {                    
	         ...
	         double dphi  = Angles::deltaPhi(phi,pphi);
	         double adphi = std::abs(dphi);
                 sumphi += phi; //do we want the recalculated phi here, should not 
