...
//
// $Id: RobustHelixFinder2_module.cc,v 1.2 2014/08/30 12:19:38 tassiell Exp $
// $Author: tassiell $
...
namespace mu2e
{
  ...
  class RobustHelixFinder2 : public art::EDProducer
  {
  public:
    explicit RobustHelixFinder2(fhicl::ParameterSet const&);
    virtual ~RobustHelixFinder2();
  ...
  RobustHelixFinder2::RobustHelixFinder2(fhicl::ParameterSet const& pset) :
  ...
  RobustHelixFinder2::~RobustHelixFinder2(){}

  void RobustHelixFinder2::beginJob()
  {
     _stmva.initMVA();
     _nsmva.initMVA();
     if (_debug > 0)
...
//CALLGRIND_DUMP_STATS;
                                       ...
                                       HelixSeedCollection& outseeds, art::Event& event)
  {
    ...
    for (auto itclust=tccol.begin(); itclust != tccol.end(); ++ itclust)
    {
        const auto& tclust = *itclust;        
        HelixSeed hseed;
        hseed._t0 = tclust._t0;

        //hseed._caloCluster = tclust._caloCluster;
        size_t index = std::distance(tccol.begin(),itclust);
	   ...
	   if (_usemva)
        ...
        {
	   if (niter < _maxniter) {hseed._status.merge(TrkFitFlag::helixConverged); ++nGoodHelix;}
           //CBE Do we need to do that for the trigger? Cn live with it for now
	   updateT0(hseed,shcol,shpcol);
	   outseeds.push_back(hseed);
        } 
    }
  
    std::cout<<"Helix "<<nGoodHelix<<std::endl;
  }

  void RobustHelixFinder2::fillMVA(HelixSeed& hseed, const StrawHitCollection& shcol)
  {
  ...
  bool RobustHelixFinder2::filterHitsMVA(HelixSeed& hseed)
  {  
      bool changed(false);
      for (auto& hhit : hseed._hhits)
      {
  ...
  unsigned  RobustHelixFinder2::filterCircleHits(HelixSeed& hseed)
  {
      unsigned changed(0);
      static CLHEP::Hep3Vector zaxis(0.0,0.0,1.0); // unit in z direction
      RobustHelix& helix = hseed._helix;
  ...
  bool RobustHelixFinder2::filterHits(HelixSeed& hseed)
  {
     RobustHelix& helix = hseed._helix;
  ...
  void RobustHelixFinder2::prefilterHits(HelixSeed& hseed)
  {
      HelixHitCollection& hhits = hseed._hhits;
  ...
  void RobustHelixFinder2::updateT0(HelixSeed& hseed, const StrawHitCollection& shcol, const StrawHitPositionCollection& shpcol)
  {
  ...
  unsigned RobustHelixFinder2::hitCount(const HelixSeed& hseed)
  {
  ...
  bool RobustHelixFinder2::updateStereo(HelixSeed& hseed, const StereoHitCollection& sthcol, const StrawHitCollection& shcol)
  {
    ...
    if (_updatestereo)
    {
	  ...
	  if (hhit.flag().hasAnyProperty(stereo) && hhit.stereoHitIndex() >= 0)
	      ...
	      if (sthit.hitIndex1() == hhit.index())
	        ...
	        throw cet::exception("RECO")<<"mu2e::RobustHelixFinder2: stereo hit index not consistent"<< endl;
...
using mu2e::RobustHelixFinder2;
DEFINE_ART_MODULE(RobustHelixFinder2);
