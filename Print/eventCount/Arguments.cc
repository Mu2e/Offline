...
mu2e::Arguments::Arguments(int argc, char** argv) : fileNames() {
  int i = 1;
...
void mu2e::Arguments::usage() const {
  cerr << "usage: eventCount [--full] [--minimal] filename1 [filename2] "
          "[filename3] ...\n\n"
       ...
       << " --subruns - print run/subrun for all subruns, including those with "
          "no events \n"
       ...
       << "   BAD means that the file could not be opened or that one or more "
          "TTrees is missing.\n"
