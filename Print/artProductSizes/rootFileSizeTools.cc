...
Long64_t mu2e::GetBasketSize(TObjArray *branches, bool ondisk, bool inclusive) {
  ...
  for (size_t i = 0; i < n; ++i) {
    result += GetBasketSize(dynamic_cast<TBranch *>(branches->At(i)), ondisk,
                            inclusive);
  }
  return result;
}

Long64_t mu2e::GetBasketSize(TBranch *b, bool ondisk, bool inclusive) {
  Long64_t result = 0;
  if (b) {
...
Long64_t mu2e::GetTotalSize(TBranch *br, bool ondisk, bool inclusive) {
...
Long64_t mu2e::GetTotalSize(TObjArray *branches, bool ondisk) {
  Long64_t result = 0;
  size_t n = branches->GetEntries();
  for (size_t i = 0; i < n; ++i) {
    result +=
        GetTotalSize(dynamic_cast<TBranch *>(branches->At(i)), ondisk, true);
    cerr << "After " << branches->At(i)->GetName() << " " << result << endl;
  }
  return result;
}

Long64_t mu2e::GetTotalSize(TTree *t, bool ondisk) {
  TKey *key = 0;
  if (t->GetDirectory()) {
  ...
  if (ondisk) {
    return ondiskSize + GetBasketSize(t->GetListOfBranches(), /* ondisk */ true,
                                      /* inclusive */ true);
  } else {
    return totalSize + GetBasketSize(t->GetListOfBranches(), /* ondisk */ false,
                                     /* inclusive */ true);
  }
}

Long64_t mu2e::sizeOnDisk(TTree *t) {
  // Return the size on disk on this TTree.

  return GetTotalSize(t, true);
}

Long64_t mu2e::sizeOnDisk(TBranch *branch, bool inclusive) {
...
void mu2e::printBranchSummary(TBranch *br) {
  cout << "The branch \"" << br->GetName() << "\" takes "
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    cout << "  It's sub-branch \"" << subbr->GetName() << "\" takes "
         << sizeOnDisk(subbr, true) << " bytes on disk\n";
  }
}

void mu2e::printTreeSummary(TTree *t) {
  cout << "The TTree \"" << t->GetName() << "\" takes " << sizeOnDisk(t)
       << " bytes on disk\n";
  size_t n = t->GetListOfBranches()->GetEntries();
  for (size_t i = 0; i < n; ++i) {
    TBranch *br = dynamic_cast<TBranch *>(t->GetListOfBranches()->At(i));
    cout << "  It's branch \"" << br->GetName() << "\" takes "
         << sizeOnDisk(br, true) << " bytes on disk\n";
  }
}
