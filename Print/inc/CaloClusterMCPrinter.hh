...
class CaloClusterMCPrinter : public ProductPrinter {
 public:
  ...
  void Print(const art::ValidHandle<CaloClusterMCCollection>& handle,
             std::ostream& os = std::cout);
  void Print(const CaloClusterMCCollection& coll, std::ostream& os = std::cout);
  void Print(const art::Ptr<CaloClusterMC>& ptr, int ind = -1,
             std::ostream& os = std::cout);
  void Print(const mu2e::CaloClusterMC& obj, int ind = -1,
             std::ostream& os = std::cout);
