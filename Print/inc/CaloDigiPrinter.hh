...
class CaloDigiPrinter : public ProductPrinter {
 public:
  ...
  void Print(const art::ValidHandle<CaloDigiCollection>& handle,
             std::ostream& os = std::cout);
  void Print(const CaloDigiCollection& coll, std::ostream& os = std::cout);
  void Print(const art::Ptr<CaloDigi>& ptr, int ind = -1,
             std::ostream& os = std::cout);
  void Print(const mu2e::CaloDigi& obj, int ind = -1,
             std::ostream& os = std::cout);
