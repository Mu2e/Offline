...
class CrvDigiMCPrinter : public ProductPrinter {
 public:
  ...
  void Print(const art::ValidHandle<CrvDigiMCCollection>& handle,
             std::ostream& os = std::cout);
  void Print(const CrvDigiMCCollection& coll, std::ostream& os = std::cout);
  void Print(const art::Ptr<CrvDigiMC>& ptr, int ind = -1,
             std::ostream& os = std::cout);
  void Print(const mu2e::CrvDigiMC& obj, int ind = -1,
             std::ostream& os = std::cout);
