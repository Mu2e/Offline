...
class MCTrajectoryPrinter : public ProductPrinter {
 public:
  ...
  void Print(const art::ValidHandle<MCTrajectoryCollection>& handle,
             std::ostream& os = std::cout);
  void Print(const MCTrajectoryCollection& coll, std::ostream& os = std::cout);
  void Print(const art::Ptr<MCTrajectory>& ptr, int ind = -1,
             std::ostream& os = std::cout);
  void Print(const mu2e::MCTrajectory& obj, int ind = -1,
             std::ostream& os = std::cout);
