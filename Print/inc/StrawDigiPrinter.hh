...
class StrawDigiPrinter : public ProductPrinter {
 public:
  ...
  void Print(const art::ValidHandle<StrawDigiCollection>& handle,
             std::ostream& os = std::cout);
  void Print(const StrawDigiCollection& coll, std::ostream& os = std::cout);
  void Print(const art::Ptr<StrawDigi>& ptr, int ind = -1,
             std::ostream& os = std::cout);
  void Print(const mu2e::StrawDigi& obj, int ind = -1,
             std::ostream& os = std::cout);
