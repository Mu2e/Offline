...
class StepPointMCPrinter : public ProductPrinter {
 public:
  ...
  void Print(const art::ValidHandle<StepPointMCCollection>& handle,
             std::ostream& os = std::cout);
  void Print(const StepPointMCCollection& coll, std::ostream& os = std::cout);
  void Print(const art::Ptr<StepPointMC>& ptr, int ind = -1,
             std::ostream& os = std::cout);
  void Print(const mu2e::StepPointMC& obj, int ind = -1,
             std::ostream& os = std::cout);
