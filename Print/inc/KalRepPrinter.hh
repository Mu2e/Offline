...
class KalRepPrinter : public ProductPrinter {
 public:
  ...
  void Print(const art::ValidHandle<KalRepPtrCollection>& handle,
             std::ostream& os = std::cout);
  void Print(const KalRepPtrCollection& coll, std::ostream& os = std::cout);
  void Print(const art::Ptr<KalRep>& ptr, int ind = -1,
             std::ostream& os = std::cout);
  void Print(const KalRep& obj, int ind = -1, std::ostream& os = std::cout);
