 ...
 public:
  struct Config : public ProductPrinter::Config {
    fhicl::Atom<float> pCut{fhicl::Name("pCut"),
                            fhicl::Comment("momentum cut on all particles"),
                            -1};
    fhicl::Atom<float> emPCut{fhicl::Name("emPcut"),
                              fhicl::Comment("momentum cut on EM particles"),
                              -1};
    fhicl::Atom<bool> primaryOnly{
        fhicl::Name("primaryOnly"),
  ...
  void Print(const art::ValidHandle<SimParticleCollection>& handle,
             std::ostream& os = std::cout);
  void Print(const SimParticleCollection& coll, std::ostream& os = std::cout);
  void Print(const art::Ptr<SimParticle>& ptr, int ind = -1,
             std::ostream& os = std::cout);
  void Print(const mu2e::SimParticle& obj, int ind = -1, std::size_t key = -1,
             std::ostream& os = std::cout);
