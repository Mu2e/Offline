...
class CrvDigiPrinter : public ProductPrinter {
 public:
  ...
  void Print(const art::ValidHandle<CrvDigiCollection>& handle,
             std::ostream& os = std::cout);
  void Print(const CrvDigiCollection& coll, std::ostream& os = std::cout);
  void Print(const art::Ptr<CrvDigi>& ptr, int ind = -1,
             std::ostream& os = std::cout);
  void Print(const mu2e::CrvDigi& obj, int ind = -1,
             std::ostream& os = std::cout);
