...
class StrawDigiMCPrinter : public ProductPrinter {
 public:
  ...
  void Print(const art::ValidHandle<StrawDigiMCCollection>& handle,
             std::ostream& os = std::cout);
  void Print(const StrawDigiMCCollection& coll, std::ostream& os = std::cout);
  void Print(const art::Ptr<StrawDigiMC>& ptr, int ind = -1,
             std::ostream& os = std::cout);
  void Print(const mu2e::StrawDigiMC& obj, int ind = -1,
             std::ostream& os = std::cout);
