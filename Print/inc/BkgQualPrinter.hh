...
class BkgQualPrinter : public ProductPrinter {
 public:
  ...
  void Print(const art::ValidHandle<BkgQualCollection>& handle,
             std::ostream& os = std::cout);
  void Print(const BkgQualCollection& coll, std::ostream& os = std::cout);
  void Print(const art::Ptr<BkgQual>& ptr, int ind = -1,
             std::ostream& os = std::cout);
  void Print(const mu2e::BkgQual& obj, int ind = -1,
             std::ostream& os = std::cout);
