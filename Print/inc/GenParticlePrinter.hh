...
class GenParticlePrinter : public ProductPrinter {
 public:
  ...
  void Print(const art::ValidHandle<GenParticleCollection>& handle,
             std::ostream& os = std::cout);
  void Print(const GenParticleCollection& coll, std::ostream& os = std::cout);
  void Print(const art::Ptr<GenParticle>& ptr, int ind = -1,
             std::ostream& os = std::cout);
  void Print(const mu2e::GenParticle& obj, int ind = -1,
             std::ostream& os = std::cout);
