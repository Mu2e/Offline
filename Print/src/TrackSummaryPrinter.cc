...
void mu2e::TrackSummaryPrinter::Print(art::Event const& event,
                                      std::ostream& os) {
  if (verbose() < 1) return;
  if (tags().empty()) {
    // if a list of instances not specified, print all instances
    std::vector<art::Handle<TrackSummaryCollection> > vah =
        event.getMany<TrackSummaryCollection>();
...
void mu2e::TrackSummaryPrinter::Print(
    const art::Handle<TrackSummaryCollection>& handle, std::ostream& os) {
  if (verbose() < 1) return;
...
void mu2e::TrackSummaryPrinter::Print(
    const art::ValidHandle<TrackSummaryCollection>& handle, std::ostream& os) {
  if (verbose() < 1) return;
  // the product tags with all four fields, with underscores
  std::string tag = handle.provenance()->productDescription().branchName();
  tag.pop_back();  // remove trailing dot
...
void mu2e::TrackSummaryPrinter::Print(const TrackSummaryCollection& coll,
                                      std::ostream& os) {
  if (verbose() < 1) return;
...
void mu2e::TrackSummaryPrinter::Print(const art::Ptr<TrackSummary>& obj,
                                      int ind, std::ostream& os) {
  if (verbose() < 1) return;
  Print(*obj, ind);
}

void mu2e::TrackSummaryPrinter::Print(const mu2e::TrackSummary& obj, int ind,
                                      std::ostream& os) {
  if (verbose() < 1) return;
  ...
  if (obj.states().size() == 0) {
    os << " a track with fitstatus " << obj.fitstatus() << " has no trajectory"
       << std::endl;
    return;
  }
  ...
  if (verbose() == 1) {
    os << " " << std::setw(5) << obj.fitstatus() << " " << std::setw(8)
       << std::setprecision(3) << obj.fitcon() << " " << std::setw(5)
       << obj.charge() << " " << std::setw(8) << std::setprecision(1)
       << state.momentum().mag() << " " << std::setw(8) << std::setprecision(3)
       << state.momentumError() << " " << std::setw(8) << std::setprecision(4)
       << helix.tanDip() << " " << std::setw(7) << std::setprecision(1)
       << helix.d0() << " " << std::setw(7) << std::setprecision(1)
       << helix.dOut() << " " << std::setw(7) << std::setprecision(1)
       << obj.t0() << " " << std::setw(7) << std::setprecision(4)
       << obj.nactive() << std::endl;
  } else if (verbose() >= 2) {
    os << "  "
    ...
    for (const auto& st : obj.states()) {
      ...
      os << "  "
         ...
         << st.position().x() << " " << std::setw(8) << std::setprecision(1)
         << st.position().y() << " " << std::setw(8) << std::setprecision(1)
         << st.position().z() << "  mom: " << std::setw(8)
         << std::setprecision(3) << st.momentum().x() << " " << std::setw(8)
         << std::setprecision(3) << st.momentum().y() << " " << std::setw(8)
         << std::setprecision(3) << st.momentum().z() << "\n";
      os << "  momErr: " << std::setw(8) << std::setprecision(3)
         << st.momentumError() << "  arrtime: " << std::setw(8)
         << std::setprecision(1) << st.arrivalTime()
         << "  fltTime: " << std::setw(8) << std::setprecision(1)
         << st.flightLength() << "  costh: " << std::setw(8)
         << std::setprecision(4) << st.costh() << "\n";
      if (verbose() >= 3) {
        os << "     d0: " << std::setw(8) << std::setprecision(1) << hh.d0()
   ...
   det->toDetector(vdg->getGlobal(VirtualDetectorId::TT_FrontPA)); double zent =
   entpos.z(); double firsthitfltlen =
   krep->firstHit()->kalHit()->hit()->fltLen(); double lasthitfltlen =
   krep->lastHit()->kalHit()->hit()->fltLen(); double entlen =
   std::min(firsthitfltlen,lasthitfltlen);
          if(!TrkHelixUtils::findZFltlen(krep->traj(),zent,entlen,0.1)) {
            throw cet::exception("RUNTIME")<<"Error from findZFltlen()\n";
          }

          double loclen(0.0);
          TrackSummary::HelixParams
   helix(*krep->localTrajectory(entlen,loclen)); TrackSummary::TrackStateAtPoint
   st(helix, fromHepPoint(krep->position(entlen)), krep->momentum(entlen),
...
void mu2e::TrackSummaryPrinter::PrintHeader(const std::string& tag,
                                            std::ostream& os) {
  if (verbose() < 1) return;
  os << "\nProductPrint " << tag << "\n";
}

void mu2e::TrackSummaryPrinter::PrintListHeader(std::ostream& os) {
  if (verbose() < 1) return;
  os << "ind  status   fitcon  chrg     p      pErr   tanDip     d0     dMax   "
        "  t0    nActive\n";
}
