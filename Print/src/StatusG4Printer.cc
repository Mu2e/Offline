...
void mu2e::StatusG4Printer::Print(art::Event const& event, std::ostream& os) {
  if (verbose() < 1) return;
  if (tags().empty()) {
...
void mu2e::StatusG4Printer::Print(const art::Handle<StatusG4>& handle,
                                  std::ostream& os) {
  if (verbose() < 1) return;
...
void mu2e::StatusG4Printer::Print(const art::ValidHandle<StatusG4>& handle,
                                  std::ostream& os) {
  if (verbose() < 1) return;
  // the product tags with all four fields, with underscores
  std::string tag = handle.provenance()->productDescription().branchName();
...
void mu2e::StatusG4Printer::Print(const mu2e::StatusG4& obj, int ind,
                                  std::ostream& os) {
  if (verbose() < 1) return;

  os << std::setiosflags(std::ios::fixed | std::ios::right);

  os << "  status: " << std::setw(3) << obj.status()
...
void mu2e::StatusG4Printer::PrintHeader(const std::string& tag,
                                        std::ostream& os) {
  if (verbose() < 1) return;
  os << "\nProductPrint " << tag << "\n";
}
