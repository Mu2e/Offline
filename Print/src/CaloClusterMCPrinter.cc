...
void mu2e::CaloClusterMCPrinter::Print(art::Event const& event,
                                       std::ostream& os) {
  if (verbose() < 1) return;
  if (tags().empty()) {
    // if a list of instances not specified, print all instances
    std::vector<art::Handle<CaloClusterMCCollection> > vah =
        event.getMany<CaloClusterMCCollection>();
...
void mu2e::CaloClusterMCPrinter::Print(
    const art::Handle<CaloClusterMCCollection>& handle, std::ostream& os) {
  if (verbose() < 1) return;
...
void mu2e::CaloClusterMCPrinter::Print(
    const art::ValidHandle<CaloClusterMCCollection>& handle, std::ostream& os) {
  if (verbose() < 1) return;
  // the product tags with all four fields, with underscores
  std::string tag = handle.provenance()->productDescription().branchName();
  tag.pop_back();  // remove trailing dot
...
void mu2e::CaloClusterMCPrinter::Print(const CaloClusterMCCollection& coll,
                                       std::ostream& os) {
  if (verbose() < 1) return;
...
void mu2e::CaloClusterMCPrinter::Print(const art::Ptr<CaloClusterMC>& obj,
                                       int ind, std::ostream& os) {
  if (verbose() < 1) return;
  Print(*obj, ind);
}

void mu2e::CaloClusterMCPrinter::Print(const mu2e::CaloClusterMC& obj, int ind,
                                       std::ostream& os) {
  if (verbose() < 1) return;
  ...
  if (verbose() == 1) {
    os << " " << std::setw(5) << obj.caloHitMCs().size() << " " << std::setw(5)
       << obj.energyDeposits().size() << " "
...
void mu2e::CaloClusterMCPrinter::PrintHeader(const std::string& tag,
                                             std::ostream& os) {
  if (verbose() < 1) return;
  os << "\nProductPrint " << tag << "\n";
}

void mu2e::CaloClusterMCPrinter::PrintListHeader(std::ostream& os) {
  if (verbose() < 1) return;
