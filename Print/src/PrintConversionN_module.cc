//
// Inspect the output of the StoppedMuonConversionGunN module.
//
...
namespace mu2e {
  ...
  class PrintConversionN : public art::EDAnalyzer {
  public:

    explicit PrintConversionN(fhicl::ParameterSet const& pset);
...
mu2e::PrintConversionN::PrintConversionN(fhicl::ParameterSet const& pset) :
  art::EDAnalyzer(pset)
  , gensTag_( pset.get<std::string>("gensTag") ){
}

void mu2e::PrintConversionN::analyze(const art::Event& event) {
...
DEFINE_ART_MODULE(mu2e::PrintConversionN);
