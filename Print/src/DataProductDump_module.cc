...
class mu2e::DataProductDumpDetail {
 public:
  ...
  void processEventProvenance(art::Provenance const& prov) {
    eventProvenances_.push_back(prov);
  }
  ...
  void processSubRunProvenance(art::Provenance const& prov) {
    subRunProvenances_.push_back(prov);
  }
  ...
  void processRunProvenance(art::Provenance const& prov) {
    runProvenances_.push_back(prov);
  }
...
mu2e::DataProductDumpDetail::DataProductDumpDetail(
    fhicl::ParameterSet const& pset) :
    eventProvenances_(),
    subRunProvenances_(), runProvenances_() {}

void mu2e::DataProductDumpDetail::process(std::string const& label,
                                          provs_type const& provs) const {
  std::cout << "Found " << provs.size() << " data products in this " << label
            << std::endl;
...
void mu2e::DataProductDumpDetail::print(provs_type const& provs) const {
  ...
  for (provs_type::const_iterator i = provs.begin(); i != provs.end(); ++i) {
  ...
  // Print table header.
  cout << setw(maxlclass) << head0 << "  " << setw(maxlmodule) << head1 << "  "
       << setw(maxlinstance) << head2 << "  " << setw(maxlprocess) << head3
       << "     " << setw(maxlprodId) << head4 << endl;

  // Printer body of header.
  for (provs_type::const_iterator i = provs.begin(); i != provs.end(); ++i) {
    art::Provenance const& prov = *i;
    cout << setw(maxlclass) << prov.friendlyClassName() << "  "
         << setw(maxlmodule) << prov.moduleLabel() << "  " << setw(maxlinstance)
         << prov.productInstanceName() << "  " << setw(maxlprocess)
         << prov.processName() << "  " << setw(maxlprodId) << prov.productID()
         << endl;
  }
