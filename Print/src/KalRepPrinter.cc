...
void mu2e::KalRepPrinter::Print(art::Event const& event, std::ostream& os) {
  if (verbose() < 1) return;
  if (tags().empty()) {
    // if a list of instances not specified, print all instances
    std::vector<art::Handle<KalRepPtrCollection> > vah =
        event.getMany<KalRepPtrCollection>();
...
void mu2e::KalRepPrinter::Print(const art::Handle<KalRepPtrCollection>& handle,
                                std::ostream& os) {
  if (verbose() < 1) return;
...
void mu2e::KalRepPrinter::Print(
    const art::ValidHandle<KalRepPtrCollection>& handle, std::ostream& os) {
  if (verbose() < 1) return;
  // the product tags with all four fields, with underscores
  std::string tag = handle.provenance()->productDescription().branchName();
  tag.pop_back();  // remove trailing dot
...
void mu2e::KalRepPrinter::Print(const KalRepPtrCollection& coll,
                                std::ostream& os) {
  if (verbose() < 1) return;
...
void mu2e::KalRepPrinter::Print(const art::Ptr<KalRep>& obj, int ind,
                                std::ostream& os) {
  if (verbose() < 1) return;
  Print(*obj, ind);
}

void mu2e::KalRepPrinter::Print(const KalRep& obj, int ind, std::ostream& os) {
  if (verbose() < 1) return;
  ...
  TrackSummary sum(krep->fitStatus().success(), krep->charge(), krep->nActive(),
                   krep->nDof(), krep->chisq(), krep->t0().t0(),
                   krep->t0().t0Err(), krep->flt0());
  ...
  TrackSummary::TrackStateAtPoint st(
      helix, fromHepPoint(krep->position(entlen)), krep->momentum(entlen),
      krep->momentumErr(entlen).covMatrix(), krep->arrivalTime(entlen), entlen);
...
void mu2e::KalRepPrinter::PrintHeader(const std::string& tag,
                                      std::ostream& os) {
  if (verbose() < 1) return;
  os << "\nProductPrint " << tag << "\n";
}

void mu2e::KalRepPrinter::PrintListHeader(std::ostream& os) {
  if (verbose() < 1) return;
