...
void mu2e::PhysicalVolumePrinter::PrintSubRun(art::SubRun const& subrun,
                                              std::ostream& os) {
  if (verbose() < 1) return;
  if (tags().empty()) {
    // if a list of instances not specified, print all instances
    std::vector<art::Handle<PhysicalVolumeInfoMultiCollection> > vah =
        subrun.getMany<PhysicalVolumeInfoMultiCollection>();
...
void mu2e::PhysicalVolumePrinter::Print(
...
void mu2e::PhysicalVolumePrinter::Print(
    const PhysicalVolumeInfoSingleStage& obj, int ind, std::ostream& os) {
  if (verbose() < 1) return;
...
void mu2e::PhysicalVolumePrinter::Print(
    const std::pair<cet::map_vector_key, mu2e::PhysicalVolumeInfo>& obj,
  ...
  os << " " << std::setw(5) << i << std::setw(5) << pv.copyNo() << "  "
     << " " << std::setw(43) << pv.name() << " " << std::setw(23)
     << pv.materialName() << std::endl;
}

void mu2e::PhysicalVolumePrinter::PrintHeader(const std::string& tag,
                                              std::ostream& os) {
  if (verbose() < 1) return;
  os << "\nProductPrint " << tag << "\n";
}

void mu2e::PhysicalVolumePrinter::PrintListHeader(std::ostream& os) {
  if (verbose() < 1) return;
  os << "ind    count\n";
}

void mu2e::PhysicalVolumePrinter::PrintPVListHeader(std::ostream& os) {
  if (verbose() < 1) return;
  os << " ind    id copyNo                                     name            "
        "        material\n";
}
