...
public:
  ...
  struct Config {
    ...
    fhicl::Table<mu2e::CaloRawWFProcessor::Config> proc_raw_conf{Name("RawProcessor"),
                                                                 Comment("Raw processor config")};
    fhicl::Table<mu2e::CaloTemplateWFProcessor::Config> proc_templ_conf{
        Name("TemplateProcessor"), Comment("Log normal fit processor config")};
    fhicl::Atom<art::InputTag> caloDigiCollection{Name("caloDigiCollection"),
                                                  Comment("Calo Digi module label")};
    fhicl::Atom<art::InputTag> pbtTag{Name("ProtonBunchTimeTag"),
                                      Comment("ProtonBunchTime producer")};
    fhicl::Atom<bool> usePBT{Name("UseProtonBunchTime"),
                             Comment("Use the proton bunch time for T0")};
    fhicl::Atom<std::string> processorStrategy{Name("processorStrategy"),
                                               Comment("Digi reco processor name")};
  ...
  explicit CaloRecoDigiMaker(const art::EDProducer::Table<Config>& config) :
      ...
      pbtTag_{config().pbtTag()}, usePBT_{config().usePBT()},
      processorStrategy_(config().processorStrategy()), digiSampling_(config().digiSampling()),
      maxChi2Cut_(config().maxChi2Cut()), maxPlots_(config().maxPlots()),
      diagLevel_(config().diagLevel()) {
    produces<CaloRecoDigiCollection>();

    if (usePBT_) {
      pbttoken_ = consumes<ProtonBunchTime>(pbtTag_);
    }
...
//-------------------------------------------------------
// The t0 time in caloDigi has been corrected to the DR time frame for backward compatibility, so we
// only need to correct for the jitter between the DR marker and the nearest clock
void CaloRecoDigiMaker::produce(art::Event& event) {
  ...
  if (diagLevel_ > 0)
    std::cout << "[CaloRecoDigiMaker::produce] end" << std::endl;
}

//--------------------------------------------------
void CaloRecoDigiMaker::beginRun(art::Run& aRun) { waveformProcessor_->initialize(); }
...
void CaloRecoDigiMaker::extractRecoDigi(const art::ValidHandle<CaloDigiCollection>& caloDigisHandle,
  ...
  for (const auto& caloDigi : caloDigis) {
    ...
    if (diagLevel_ > 2)
      std::cout << "CaloRecoDigiMaker found " << waveformProcessor_->nPeaks()
                << " peaks for SiPMID=" << SiPMID << std::endl;
    ...
    for (int i = 0; i < waveformProcessor_->nPeaks(); ++i) {
      ...
      if (diagLevel_ > 2)
        std::cout << "Found reco digi hit with eDep=" << eDep << "  time=" << time
                  << " chi2=" << chi2 << "  ndf=" << ndf << std::endl;
      if (chi2 / float(ndf) > maxChi2Cut_)
        continue;

      if (SiPMID % 2 == 0)
        totEnergyReco += eDep;
      recoCaloHits.emplace_back(
          CaloRecoDigi(caloDigiPtr, eDep, eDepErr, time, timeErr, chi2, ndf, isPileUp));
    }
  }

  if (diagLevel_ > 1)
    std::cout << "[CaloRecoDigiMaker] Total energy reco " << totEnergyReco << std::endl;
}
