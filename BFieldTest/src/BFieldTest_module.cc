...
//
// The code looks for all parameter sets defined within the module paramter set.  Each of these sub parameter
// sets must have the format:
//
...
// directory.  The file will contain the above output for a scan along the line defined by the parameter set.
//
//
...
namespace {
  ...
  struct ScanConfig {
    ...
    ScanConfig ( std::string const&  aname, fhicl::ParameterSet const&  modulePSet):
      name(aname){
  ...
  void scanz ( ScanConfig const& c, mu2e::BFieldManager const& bfmgr ){
    ...
    double dz((c.zmax-c.zmin)/( c.npoints > 1 ? c.npoints - 1 : 1 /* dz not used for 1 point*/));
    ...
    for(int i=0; i<c.npoints; ++i) {
      ...
      std::fprintf ( file, "%15.10f %15.10f %15.10f %15.10f %15.10f %15.10f\n",
                     c.x, c.y, z, field[0], field[1], field[2] );
    }
...
namespace mu2e {
  ...
  public:
      ...
      : art::EDAnalyzer(pset)
    {
      ...
      if ( pskeys.empty() ){
        throw cet::exception("BFIELDTEST")
          << "BFieldTest: there are no sub parameter sets. Presume this is a serious error.\n";
      }
