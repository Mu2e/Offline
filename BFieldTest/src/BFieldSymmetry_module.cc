...
namespace {
  ...
  struct BFSymmetryChecker{
    ...
    BFSymmetryChecker( mu2e::BFMap const& map,
                       art::TFileService& tfs,
                       int nbins );
  ...
  struct NtEntry {
    ...
    static TString names() {
      return "x:y:z:bx1:by1:bz1:bx2:by2:bz2";
    }
  ...
  BFSymmetryChecker::BFSymmetryChecker( mu2e::BFMap const& amap,
                                        art::TFileService& tfs,
                                        int nbins ):
    map_(amap){
  ...
  mu2e::BFMap const& getMap( std::string const& mapName ) {
    ...
    throw cet::exception("GEOM")
      << "BFieldSymmery: cannot find the map named: "
      << mapName
      << "\n";
  }
...
void mu2e::BFieldSymmetry::beginRun(const art::Run& run) {
  ...
  for ( auto const& name : mapNames_ ){
    ...
    for ( int i=0; i<nPoints_; ++i ){
      ...
      if ( !stat1 ){
        throw cet::exception("GEOM")
          << "BFieldSymmery: bad status from map "
          << name
          << " for the point: "
          << p1
          << "\n";
...
CLHEP::Hep3Vector mu2e::BFieldSymmetry::fire( mu2e::BFMap const& map) {
  CLHEP::Hep3Vector val( flat_.fire( map.xmin(), map.xmax() ),
                         flat_.fire( map.ymin(), map.ymax() ),
