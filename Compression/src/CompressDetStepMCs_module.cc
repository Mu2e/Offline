...
public:
  ...
  struct Config {
    ...
    fhicl::Sequence<art::InputTag> simParticleTags{Name("simParticleTags"), Comment("Sequence of InputTags for the SimParticleCollections we need to compress")};
  ...
  _mcTrajectoryToken{mayConsume<mu2e::MCTrajectoryCollection>(_conf.mcTrajectoryTag())}
{
  ...
  for (const auto& i_tag : _conf.simParticleTags()) {
...
void mu2e::CompressDetStepMCs::compressSimParticles(const art::Event& event) {
  ...
  KeyRemap* keyRemap = new KeyRemap; // we will be rekeying the SimParticleCollection
  for (const auto& i_tag : _conf.simParticleTags()) {
    keyRemap->clear();
