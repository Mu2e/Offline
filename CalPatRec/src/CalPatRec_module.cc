...
//-----------------------------------------------------------------------------
  void CalPatRec::produce(art::Event& event ) {
    ...
    ::KalRep*                 krep;
                                                 // dummy objects, use for initialization
    static TrkDefHack         dummy_trkdef;

    static HelixFitHackResult dummyhfit(dummy_trkdef);
    static KalFitResult       dummykfit(&dummy_trkdef);
...
//-----------------------------------------------------------------------------
      TrkDefHack   seeddef(tp,_shcol,_shpcol,_flags,_tpart,_fdir);
      TrkDefHack   kaldef (seeddef);
                                        // track fitting objects for this peak

      HelixFitHackResult hf_result(seeddef);
...
//-----------------------------------------------------------------------------
  void CalPatRec::filterOutliers(TrkDefHack&                mytrk  ,
    ...
    const StrawHitCollection* hits = mytrk.shcol();
