...
public:
  struct Config {
    ...
    fhicl::Atom<art::InputTag> chCollLabel{Name("chCollLabel"),
                                           Comment("combo hit collection label")};
    ...
    fhicl::Atom<bool> findMultipleHelices{Name("findMultipleHelices"),
                                          Comment("allow more than one helix")};
    fhicl::Atom<bool> useStoppingTarget{Name("useStoppingTarget"),
                                        Comment("allow in triplet candidates")};
    fhicl::Atom<int> intenseEventThresh{Name("intenseEventThresh"),
    ...
    fhicl::Atom<float> minTripletSeedZ{Name("minTripletSeedZ"),
                                       Comment("minimum z for triplet seed")};
    ...
    fhicl::Atom<float> minTripletDist{Name("minTripletDist"),
                                      Comment("min XY dist btwn 2 trip pts")};
    fhicl::Atom<float> minTripletArea{Name("minTripletArea"), Comment("triangle area of triplet")};
    fhicl::Atom<float> maxSeedCircleResidual{Name("maxSeedCircleResidual"),
    ...
    fhicl::Atom<float> maxPhiZResidual{Name("maxPhiZResidual"),
                                       Comment("when refining phi-z line")};
    fhicl::Atom<int> minFinalSeedHits{Name("minFinalSeedHits"),
                                      Comment("halt search if below thresh")};
    fhicl::Atom<float> maxCircleRecoverSigma{Name("maxCircleRecoverSigma"),
                                             Comment("when doing final recovery")};
    fhicl::Atom<float> maxLineRecoverSigma{Name("maxLineRecoverSigma"),
                                           Comment("when doing final recovery")};
    fhicl::Atom<float> caloClusterSigma{Name("caloClusterSigma"),
                                        Comment("error assigned to calo clust")};
    fhicl::Atom<int> minNHelixStrawHits{Name("minNHelixStrawHits"),
                                        Comment("straw hit save threshold")};
    fhicl::Atom<int> minNHelixComboHits{Name("minNHelixComboHits"),
                                        Comment("combo hit save threshold")};
    fhicl::Atom<float> minHelixPerpMomentum{Name("minHelixPerpMomentum"),
                                            Comment("min pt of helix")};
    fhicl::Atom<float> maxHelixPerpMomentum{Name("maxHelixPerpMomentum"),
                                            Comment("max pt of helix")};
    ...
    fhicl::Atom<float> chi2LineSaveThresh{Name("chi2LineSaveThresh"),
                                          Comment("max chi2Dof for line")};
    ...
    fhicl::Atom<int> debugStrawHitThresh{Name("debugStrawHitThresh"),
                                         Comment("min #SHs for plot particle")};
    fhicl::Atom<float> debugScatterThresh{Name("debugScatterThresh"),
...
AgnosticHelixFinder::AgnosticHelixFinder(const art::EDProducer::Table<Config>& config) :
    ...
    _minHelixMomentum(config().minHelixMomentum()), _maxHelixMomentum(config().maxHelixMomentum()),
    _chi2LineSaveThresh(config().chi2LineSaveThresh()), _debugPdgID(config().debugPdgID()),
    _debugMomentum(config().debugMomentum()), _debugDirection(config().debugDirection()),
...

{
  ...
  if (_debug == 1) {
    ...
    if (_runDisplay == 1) {
      ...
      _phiz2 = new TCanvas(
          "_phiz2", "Phi-Z view after computing phi relative to circle center (stage 2)", 900, 900);
      ...
      _xy4 =
          new TCanvas("_xy4", "XY view of final circle after recovery stage (stage 4)", 900, 900);
      _phiz4 = new TCanvas("_phiz4", "Phi-z view of final line after recovery stage (stage 4)", 900,
                           900);
    }
  } else {
  ...
  if (_diagLevel == 1)
    _hmanager = art::make_tool<ModuleHistToolBase>(config().diagPlugin, "diagPlugin");
  else
    _hmanager = std::make_unique<ModuleHistToolBase>();

  if (_useStoppingTarget == true) {
    _stopTargPos.SetCoordinates(0.0, 0.0, std::numeric_limits<float>::max());
  }
}
