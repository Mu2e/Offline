...
// assume that the input is 'makeSH'
//
// $Id: MakeStrawHitsHack_module.cc,v 1.1 2014/02/23 00:56:35 murat Exp $
// $Author: murat $
...
namespace mu2e {
  ...
  class MakeStrawHitsHack : public art::EDProducer {

  public:
    explicit MakeStrawHitsHack(fhicl::ParameterSet const& pset);
    // Accept compiler written d'tor.
    virtual ~MakeStrawHitsHack()
    {
      delete _cluTool;
    }
 ...
 // Parameters
  ...
  void MakeStrawHitsHack::doTimeDistance(double zcal, double zstraw, double &distance)
  {
  ...
  MakeStrawHitsHack::MakeStrawHitsHack(fhicl::ParameterSet const& pset) :
    ...
    _oldHits(0),_newHits(0)
  {
   ...
   void MakeStrawHitsHack::beginJob(){
  ...
  void
  MakeStrawHitsHack::produce(art::Event& event) {

    if ( _diagLevel > 0 && (event.id().event() % 100 ==0)  ) cout << "MakeStrawHitsHack: produce() begin; event " << event.id().event() << endl;
    ...
    if(strawhits == 0){
      throw cet::exception("RECO")<<"mu2e::MakeStrawHitsHack: No StrawHit collection found for label " <<  _shLabel << endl;
    } 
  ...
  } // end MakeStrawHitsHack::produce.

} // end namespace mu2e

using mu2e::MakeStrawHitsHack;
DEFINE_ART_MODULE(MakeStrawHitsHack)
