#ifndef STMTemplateWFProcessor_HH
#define STMTemplateWFProcessor_HH

// This is an signal extraction method based on the adcs template.
//
// Each peak in the adcs is described by two parameters: amplitide and peak time
// For a single peak, the amplitude can be found analytically for a given start time, and a
// quasi-Netwon method can be used to fit the adcs.
...
// timing accuracy
//
// A peak inside a adcs is defined to be a pile-up if it is on top of another peak.
...
namespace mu2e {
  ...
  class STMTemplateWFProcessor : public STMWaveformProcessor
  {
     public:
        struct Config
        {
        ...
        STMTemplateWFProcessor(const Config& config);
