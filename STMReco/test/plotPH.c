...
//
void plotPH(std::string filename = "ph.root") {
  ...
  TH1F* hRawWaveform = (TH1F*) file->Get(("phLaBr/h_waveform_"+evt_wvf).c_str());
  ...
  TH1F* hDeconvolved = (TH1F*) file->Get(("phLaBr/h_deconvolved_"+evt_wvf).c_str());
  hDeconvolved->SetLineColor(kOrange);
  hDeconvolved->Draw("HIST SAME");
  leg->AddEntry(hDeconvolved, "deconvolution (tau parameter)", "l");

  TH1F* hDifferentiated = (TH1F*) file->Get(("phLaBr/h_differentiated_"+evt_wvf).c_str());
  ...
  TH1F* hAveraged = (TH1F*) file->Get(("phLaBr/h_averaged_"+evt_wvf).c_str());
  hAveraged->SetLineColor(kRed);
  hAveraged->SetLineWidth(2);
  hAveraged->Draw("HIST SAME");
  leg->AddEntry(hAveraged, "average (L parameter)", "l");

  TH1F* hBaselineMean = (TH1F*) file->Get(("phLaBr/h_baseline_mean_"+evt_wvf).c_str());
  ...
  TH1F* hPeaks = (TH1F*) file->Get(("phLaBr/h_peaks_"+evt_wvf).c_str());
  hPeaks->SetLineColor(kBlue);
  hPeaks->SetLineWidth(2);
  hPeaks->Draw("HIST SAME");
  leg->AddEntry(hPeaks, "found peaks", "l");

  TH1F* hPeakThresh = (TH1F*) file->Get(("phLaBr/h_peak_threshold_"+evt_wvf).c_str());
