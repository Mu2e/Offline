  ...
  void STMZeroSuppression::beginJob() {
    ...
    for (const auto& digi : *digisHandle) {
      ...
      for (size_t i = 0; i < starts.size(); ++i) {
        ...
        STMDigi stm_digi(digi.trigNum(), STMTrigType(digi.trigType().mode(), digi.trigType().channel(), STMDataType::kZeroSuppressed), digi.trigTime()+_ctPerNs*start, 0, 0, 0, 0, zp_adcs);
