...
#include "Offline/STMReco/inc/STMTemplateWFUtil.hh"
...
namespace mu2e {
   ...
   STMTemplateWFProcessor::STMTemplateWFProcessor(const Config& config) :
   ...
   void STMTemplateWFProcessor::initialize()
   {
       fmutil_.initialize();
   }


   void STMTemplateWFProcessor::extract(const std::vector<double>& xInput, const std::vector<double>& yInput)
   {
       if (diagLevel_>2) std::cout<<"STMTemplateWFProcessor start"<<std::endl;
       ...
       for (unsigned i=0;i<fmutil_.nPeaks();++i)
       {
           ...
           //modified estimate for the uncertainty, see note in STMTemplateWFUtil
   ...
   //----------------------------------------------------------------------------------------------------------------------------------
   void STMTemplateWFProcessor::reset() {resAmp_.clear(); resAmpErr_.clear(); resTime_.clear(); resTimeErr_.clear();ndf_ = 0;chi2_ = 999; fmutil_.reset();}

   //---------------------------------------------------------------------------------------------------------------------------------------------
   void STMTemplateWFProcessor::setPrimaryPeakPar1(const std::vector<double>& xvec, const std::vector<double>& yvecOrig)
   {
        ...
        if (diagLevel_>1) std::cout<<"[STMTemplateWFProcessor] Noise level "<<noise<<std::endl;
        ...
        for (auto i=windowPeak_;i+windowPeak_<xvec.size();++i)
        {
            ...
            if (diagLevel_>1) std::cout<<"[STMTemplateWFProcessor] Found primary peak "<<xmaxEstimate<<std::endl;
        }
        ...
        if (fmutil_.nPeaks()==0)
        {
            ...
            if (ipeak+1 < ywork.size())
            {
                ...
                fmutil_.setPar(parInit);
                if (diagLevel_>1) std::cout<<"[STMTemplateWFProcessor] Found primary peak alt "<<xvec[ipeak] <<std::endl;
            }
        }
   ...
   //---------------------------------------------------------------------------------------------------------------------------------------------
   void STMTemplateWFProcessor::setPrimaryPeakPar2(const std::vector<double>& xvec, const std::vector<double>& yvecOrig)
   {
        if (windowPeak_ > xvec.size()) return;
        std::vector<double> parInit{},yvec(yvecOrig);
        ...
        for (auto i=windowPeak_;i<int(xvec.size())-windowPeak_;++i)
        {
            ...
            if (diagLevel_ > 2) std::cout<<"[STMTemplateWFProcessor] Found primary peak bis "<<xmaxEstimate<<std::endl;
    ...
    //--------------------------------------------------------------------------------------------------
    void STMTemplateWFProcessor::findRisingPeak(int ipeak, std::vector<double>& parInit, const std::vector<double>& xvec,
    ...
    //--------------------------------------------------------------------------------------------------
    void STMTemplateWFProcessor::setSecondaryPeakPar(const std::vector<double>& xvec, const std::vector<double>& yvec)
    {
        if (windowPeak_ > xvec.size()) return;
        if (xvec.size() != yvec.size()) throw cet::exception("CATEGORY")<<"STMTemplateWFProcessor::setSecondaryPeakPar  xvec and yvec must have the same size";
   ...
   //------------------------------------------------------------
   double STMTemplateWFProcessor::estimatePeakTime(const std::vector<double>& xvec, const std::vector<double>& ywork, int ic)
   {
   ...
   //------------------------------------------------------------------------------------------
   bool STMTemplateWFProcessor::checkPeakDist(double x0)
   {
       for (unsigned i=0;i<fmutil_.nPeaks();++i)
       {
           unsigned ip = fmutil_.peakIdx(i);
   ...
   //---------------------------------------------------------------------------------------------------------------------------------------
   void STMTemplateWFProcessor::plot(const std::string& name) const {fmutil_.plotFit(name);}

   //---------------------------------------------------------------------------------------------------------------------------------------
   void STMTemplateWFProcessor::dump(const std::string& name, const std::vector<double>& val) const
   {
   ...
   //---------------------------------------------------------------------------------------------------------------------------------------
   void STMTemplateWFProcessor::initHistos()
   {
       art::ServiceHandle<art::TFileService> tfs;
       art::TFileDirectory tfdir = tfs->mkdir("STMTemplateWFDiag");
...
/*
   ...
   void STMTemplateWFProcessor::setPrimaryPeakPar1(const std::vector<double>& xvec, const std::vector<double>& yvec)
   {
